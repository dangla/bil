...
int calcul(DataSet_t* jdd)
{
  ...
  int n_sequences = DataSet_GetNbOfSequences(jdd) ;
  const int n_sol = n_sequences + 2 ; /* Must be n_sequences + 2 at minimum */
...
#define STEP_1    Solution_GetStepIndex(SOL_1)
  ...
  int nbofsequences = DataSet_GetNbOfSequences(jdd) ;
  ...
  
  {
    ...
    do {
      ...
      DataSet_GetSequentialIndex(jdd) = sequentialindex ;
      ...
      if(sequentialindex > 0) {
        Solution_CopyInDistantFuture(SOL_1,sequentialindex) ;
      }
      ...
      Mesh_InterpolateCurrentUnknowns(mesh,sols,sequentialindex) ;
      ...
      if(sequentialindex < nbofsequences - 1) {
        ...
        {
          int i = Increment(jdd,sols,solver+1,outputfiles,idate,t_0,t3) ;
