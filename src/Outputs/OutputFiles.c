...
{
  double version = OutputFiles_Version(outputfiles) ;
      ...
      for(i_temps = 0 ; i_temps < n_dates ; i_temps++) {
        ...
        if(i_temps == 0) {
          ...
          for(i = 0 ; i < nviews ; i++) {
            ...
            if(nvert == 0) {
              arret("OutputFiles_PostProcessForGmshParsedFileFormat: unknown type") ;
            }
...
/* Create gmsh post-processing files in parsed file format */
{
  ...
  /* Create and open the view files for writing */
  {
    ...
    if(i > OutputFiles_MaxLengthOfFileName) {
      arret("OutputFiles_PostProcessForGmshParsedFileFormatVersion2: too lengthy name") ;
    }
  ...
  /* Record the results in the view files */
  {
    ...
    for(ie = 0 ; ie < n_el ; ie++) {
      ...
      if(nvert == 0) {
        arret("OutputFiles_PostProcessForGmshParsedFileFormatVersion2: unknown type") ;
      }
      
      if(usedmodelindex < 0) {
        arret("OutputFiles_PostProcessForGmshParsedFileFormatVersion2") ;
      }
