...
int pm(const char* s)
{
  ...
  else if(strcmp(s,"R_CH") == 0)          return (8) ;
  ...
  else if(strcmp(s,"R_CC") == 0)          return (18) ;
  else return(-1) ;
}
...
void GetProperties(Element_t* el)
{
  ...
  r_cc     = GetProperty("R_CC") ;
  r_ch     = GetProperty("R_CH") ;
  ...
  /* Pre-initialization */
  {
    ...
    for(i = 0 ; i < nn ; i++) {
      ...
      /* Solid contents */
      {
        ...
        //double n_ch       = CalciumContentInCH(zn_ca_s) ;
        double n_ch       = n_ch0 ;
        double n_cc       = 0 ;
...
void  ComputeSecondaryVariables(Element_t* el,double t,double dt,double* x)
{
  ...
  /* Solve cement chemistry */
  {
    ...
    HardenedCementChemistry_SetInput(hcc,SI_CH,zn_ca_s) ;
  ...
  double n_cc       = CCSolidContent(n_ccn,s_cc,dt) ;
...
double CHSolidContent_kin1(double n_chn,double s_ch,double dt)
{
  ...
  double dn_chsdt   = dn1sdt*(s_ch - 1) ; /* Kinetics */
