...
#define MODELINDEX  31
#define TITLE "Ecoulement a double permeabilite avec solute"
...
static int    c31(double**,double**,double**,double*,double*,double*,elem_t,geom_t,double,double*) ;
static int    k31(double**,double**,double**,double*,double*,double*,elem_t,geom_t,double,double*) ;
static double tortuosite_f(double,double) ;
static double tortuosite_m(double,double) ;
static double normv(double*) ;
/* Parametres */
static double gravite,v_f,v_m,rho,eps_f0,eps_m0,K_f,K_m,K_a,mu,p_g,S_s,p_f0,p_m0,d0_sf,d0_sm,lambda_sf,lambda_sm,alpha_s ;
...
int pm(const char *s)
{
  if(strcmp(s,"gravite") == 0) return (0) ;
    ...
    { printf("donnee \"%s\" non connue (pm31)\n",s) ; exit(0) ; }
}


int dm31(int dim,mate_t *mat,FILE *ficd)
/* Lecture des donnees materiaux dans le fichier ficd */
{
  int n_donnees = 18,n_courbes = 5 ;
...
void tb31(elem_t *el,inte_t *fi,unsigned int *n_fi,int dim)
{
  el->n_vi = NVI*el->fi->np ;
  el->n_ve = NVE*el->fi->np ;
}

void ch31(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,double *r,elem_t el,int dim,geom_t geom,double dt,double t,char_t cg)
...
void in31(double **x,double **u,double *f,double *va,elem_t el,int dim,geom_t geom)
/* Initialise les variables du systeme (f,va) */ 
{
  double p_f,p_m,s_f,s_m,m_f,m_m,dm_a,c_f,c_m,m_sf,m_sm,dm_sa ;
  ...
  */
  ...
  for(p=0;p<el.fi->np;p++) {
    ...
    m_sf = c_f*v_f*eps_f*s_f ;
    m_sm = c_m*v_m*eps_m*s_m ;
...
int ex31(double **x,double **u,double *f,double *va,elem_t el,int dim,geom_t geom,double t) 
...
int ct31(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,elem_t el,int dim,geom_t geom,double dt,double t)
...
int  mx31(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,double *k,elem_t el,int dim,geom_t geom,double dt,double t)
/* Matrice (k) */
{
  int    i,dec ;
  ...
  /*
    loi de comportement
  */
  dec = c31(x,u_1,u_n,f_1,f_n,va,el,geom,dt,c) ;
  mxcmss(k,x,*el.fi,c,dim,dec,geom,NEQ) ;
  /*
    conduction
  */
  dec = k31(x,u_1,u_n,f_1,f_n,va,el,geom,dt,c) ;
...
void rs31(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,double *r,elem_t el,int dim,geom_t geom,double dt,double t)
/* Residu (r) */
{
  int    i ;
...
int so31(double **x,double **u,double *f,double *va,double *s,resu_t *r,elem_t el,int dim,geom_t geom,double t)
...
int c31(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,elem_t el,geom_t geom,double dt,double *c)
/*
**  Matrice de comportement (c) et decalage (dec)
*/
{
  double p_f,p_m,c_f,c_m ;
  ...
  */
  ...
  for(p=0;p<el.fi->np;p++) {
    ...
    /* solute fracture */
    c1[E_sf*NEQ+I_p_f]  =   c_f*v_f*(eps_f*(-ds_fsdpc) + S_s*s_f) + dt*k_sa ;
    c1[E_sf*NEQ+I_p_m]  = - dt*k_sa ;
    c1[E_sf*NEQ+I_c_f]  =   v_f*eps_f*s_f + dt*d_sa ;
    c1[E_sf*NEQ+I_c_m]  = - dt*d_sa ;
    /* solute matrice */
    c1[E_sm*NEQ+I_p_f]  = - dt*k_sa ;
    c1[E_sm*NEQ+I_p_m]  =   c_m*v_m*(eps_m*(-ds_msdpc) + S_s*s_m) + dt*k_sa ;
    c1[E_sm*NEQ+I_c_f]  = - dt*d_sa ;
    c1[E_sm*NEQ+I_c_m]  =   v_m*eps_m*s_m + dt*d_sa ;
  }
  return(dec) ;
}


int k31(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,elem_t el,geom_t geom,double dt,double *c)
/*
