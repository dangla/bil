...
#if (U_PG == LOG_U)
  #define P_G(n)     (exp(Ln10*UNKNOWN(n,I_P_G)))
  #define P_Gn(n)    (exp(Ln10*UNKNOWNn(n,I_P_G)))
#else
  #define P_G(n)     (UNKNOWN(n,I_P_G))
  #define P_Gn(n)    (UNKNOWNn(n,I_P_G))
#endif
#if (U_PL == LOG_U)
  #define P_L(n)     (exp(Ln10*UNKNOWN(n,I_P_L)))
  #define P_Ln(n)    (exp(Ln10*UNKNOWNn(n,I_P_L)))
#else
...
#define ZN_Ca_Sn(n) (UNKNOWNn(n,I_ZN_Ca_S))
#define ZN_Si_Sn(n) (UNKNOWNn(n,I_ZN_Si_S))
#define PSIn(n)     (UNKNOWNn(n,I_PSI))
#define C_Kn(n)     (UNKNOWNn(n,I_C_K))
#define C_Cln(n)    (UNKNOWNn(n,I_C_Cl))
...
/* Charge of ions */
#include "ElectricChargeOfIons.h"
...
#define M_Cl           (MolarMassOfMolecules(Cl)*gr)

/* Partial Molar Volume of Ions (dm3/mole) */
#include "PartialMolarVolumeOfMoleculesInWater.h"
#define dm3            (1.e3)
...
/* Molar Volumes of solids (dm3/mole) */
...
#define IonActivityProductOfSH(s_ch,zn_si_s)  (K_SH*SaturationDegreeOfSH(s_ch,zn_si_s))
...
#define IonActivityProductOfCH(z_co2,zn_ca_s) (K_CH*SaturationDegreeOfCH(z_co2,zn_ca_s))
...
#define IonActivityProductOfCC(z_co2,zn_ca_s) (K_CC*SaturationDegreeOfCC(z_co2,zn_ca_s))
...
{
  K_h2o      = EquilibriumConstantOfHomogeneousReactionsInWater(H2O__H_OH,TK) ;
  K_hco3     = EquilibriumConstantOfHomogeneousReactionsInWater(CO2_H2O__HCO3_H,TK) ;
  K_co3      = EquilibriumConstantOfHomogeneousReactionsInWater(HCO3__H_CO3,TK) ;
  K_h3sio4   = EquilibriumConstantOfHomogeneousReactionsInWater(H4SiO4__H3SiO4_H,TK) ;
  K_h2sio4   = EquilibriumConstantOfHomogeneousReactionsInWater(H3SiO4_OH__H2SiO4_H2O,TK) ;
  K_cahco3   = EquilibriumConstantOfHomogeneousReactionsInWater(Ca_HCO3__CaHCO3,TK) ;
  K_caco3aq  = EquilibriumConstantOfHomogeneousReactionsInWater(Ca_CO3__CaCO3,TK) ;
  K_cah2sio4 = EquilibriumConstantOfHomogeneousReactionsInWater(Ca_H2SiO4__CaH2SiO4,TK) ;
  K_cah3sio4 = EquilibriumConstantOfHomogeneousReactionsInWater(Ca_H3SiO4__CaH3SiO4,TK) ;
  K_caoh     = EquilibriumConstantOfHomogeneousReactionsInWater(Ca_OH__CaOH,TK) ;

  K_CH = DissociationConstantOfCementHydrationProducts(CH__Ca_2OH,TK) ;
  K_CC = DissociationConstantOfCementHydrationProducts(CC__Ca_CO3,TK) ;
  K_SH = DissociationConstantOfCementHydrationProducts(S_2H2O__H4SiO4,TK) ;
  ...
  d_co2        = DiffusionCoefficientOfMoleculesInWater(CO2,TK)*dm2 ;
  ...
  d_k          = DiffusionCoefficientOfMoleculesInWater(K,TK)*dm2 ;
  d_koh        = DiffusionCoefficientOfMoleculesInWater(KOH,TK)*dm2 ;
  d_cl         = DiffusionCoefficientOfMoleculesInWater(Cl,TK)*dm2 ;
  d_na         = DiffusionCoefficientOfMoleculesInWater(Na,TK)*dm2 ;
  ...
  /* Physical constants */
  {
    double RT  = PhysicalConstants(PerfectGasConstant)*TK ;
    FsRT       = PhysicalConstants(Faraday)/RT ;
  }
}


#define NbOfComponents    (44)
...
#if (U_PG == LOG_U)
  Model_CopyNameOfUnknown(model,I_P_G,"logp_co2") ;
#else
  Model_CopyNameOfUnknown(model,I_P_G,"p_co2") ;
#endif
#if (U_PL == LOG_U)
  Model_CopyNameOfUnknown(model,I_P_L,"logp_l") ;
#else
 ...
 *  Return the nb of equations */
{
  ...
  printf("\t - Zeta unknown for silicium    (zn_si_s)\n") ;
  ...
  for(i = 0 ; i < nn ; i++) {
...

    {
      ...
      if(c_co2 < 0 || p_g < 0) {
        ...
        double n_si_s     = n_si_ref*MAX(zn_si_s,0.) ;
        double n_ca_s     = n_ca_ref*MAX(zn_ca_s,0.) + x_csh*n_si_s ;
...
#if (U_PG == LOG_U)
  for(i=0;i<2*NEQ;i++){
    K(i,I_P_G)     *= Ln10*P_G(0) ;
    K(i,I_P_G+NEQ) *= Ln10*P_G(1) ;
  }
#endif
#if (U_PL == LOG_U)
  for(i=0;i<2*NEQ;i++){
   ...
   as root of ax^4 + bx^3 + cx^2 + dx + e = 0 */
{
  ...
  double Q_CC       = IonActivityProductOfCC(z_co2,zn_ca_s) ;
  double s_ch       = SaturationDegreeOfCH(z_co2,zn_ca_s) ;
  double Q_SH       = IonActivityProductOfSH(s_ch,zn_si_s) ;
...
void  ComputeSecondaryComponents(Element_t *el,double dt,double *x)
{
  ...
  double n_ch_eq    = n_ca_ref*MAX(zn_ca_s,0.) - n_cc_ci ;
  double n_cc_eq    = n_ca_ref*MAX(zn_ca_s,0.) - n_ch_ci ;
...
double* Fluxes(Element_t *el,double *grd)
{
  ...
  double w_m     	  = - KD_M*grd_p_l 				- KD_GM*grd_p_g ;
  ...
  */
  ...
  w[I_W_C    ] = w_co2 + w_hco3 + w_co3 + w_cahco3 + w_caco3aq ;
...
/**  Tangent matrix coefficients (c) */
{
  ...
  */
  ...
  for(i = 0 ; i < 2 ; i++) {
    ...
    dxi[I_P_G    ] = P_Gn(i)*1.e-6 ;
    dxi[I_P_L    ] = P_Ln(i)*1.e-6 ;
    dxi[I_ZN_Si_S] = ((x[I_ZN_Si_S] > ZN_Si_Sn(i)) ? 1 : -1)*1.e-4 ; 
    dxi[I_ZN_Ca_S] = ((x[I_ZN_Ca_S] > ZN_Ca_Sn(i)) ? 1 : -1)*1.e-6 ;
    dxi[I_C_K    ] = 0.4e-4 ;
    dxi[I_C_Cl   ] = 0.4e-4 ;
    dxi[I_PSI    ] = 1. ;
...
/* on resout ax^4 + bx^3 + cx^2 + dx + e = 0 */
{
  ...
  double iks[5] ;
  double zed[2] ;
  double x0 = quartic_equation(a,b,c,d,e,iks,zed) ;
  double x  = MAX(x0,0.) ;
  int    i = 0 ;
  /* 
     on neglige a et c
