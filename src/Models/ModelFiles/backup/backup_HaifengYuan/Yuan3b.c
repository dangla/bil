...
#define CHSolidContent_kin(n,s,dt)     (n*pow(s,dt/t_ch))
#define CHSolidContent(zn_ca_s,s_ch,s_csh2,n_chn,n_csh2n,dt)  ((s_csh2 > s_ch) ? CHSolidContent_kin(n_chn,s_ch,dt) : CalciumContentInCHAndCSH2(zn_ca_s) - CSH2SolidContent_kin(n_csh2n,s_csh2,dt))
...
#define CSH2SolidContent_kin(n,s,dt)     (n*pow(s,dt/t_csh2))
#define CSH2SolidContent(zn_ca_s,s_ch,s_csh2,n_chn,n_csh2n,dt)  ((s_csh2 > s_ch) ? CalciumContentInCHAndCSH2(zn_ca_s) - CHSolidContent_kin(n_chn,s_ch,dt) : CSH2SolidContent_kin(n_csh2n,s_csh2,dt))
...
double* ComputeFluxes(Element_t* el,double* grd,int i,int j)
{
  ...
  /* Gradients */
  double grd_h        = grd[I_C_H]        + z_h        * grd[I_PSI] ;
  double grd_oh       = grd[I_C_OH]       + z_oh       * grd[I_PSI] ;
  
  double grd_ca       = grd[I_C_Ca]       + z_ca       * grd[I_PSI] ;
  double grd_caoh     = grd[I_C_CaOH]     + z_caoh     * grd[I_PSI] ;
  double grd_caoh2aq  = grd[I_C_CaOH2aq] ;
  
  double grd_h2sio4   = grd[I_C_H2SiO4]   + z_h2sio4   * grd[I_PSI] ;
  double grd_h3sio4   = grd[I_C_H3SiO4]   + z_h3sio4   * grd[I_PSI] ;
  double grd_h4sio4   = grd[I_C_H4SiO4] ;
  
  double grd_k        = grd[I_C_K]        + z_k        * grd[I_PSI] ;
  double grd_koh      = grd[I_C_KOH] ;
  
  double grd_h2so4    = grd[I_C_H2SO4] ;
  double grd_hso4     = grd[I_C_HSO4]     + z_hso4     * grd[I_PSI] ;
  double grd_so4      = grd[I_C_SO4]      + z_so4      * grd[I_PSI] ;
  
  double grd_cl       = grd[I_C_Cl]       + z_cl       * grd[I_PSI] ;
  
  double grd_al       = grd[I_C_Al]       + z_al       * grd[I_PSI] ;
  double grd_alo4h4   = grd[I_C_AlO4H4]   + z_alo4h4   * grd[I_PSI] ;
  
  double grd_cah3sio4 = grd[I_C_CaH3SiO4] + z_cah3sio4 * grd[I_PSI] ;
  double grd_cah2sio4 = grd[I_C_CaH2SiO4] ;
  
  double grd_cahso4   = grd[I_C_CaHSO4]   + z_cahso4   * grd[I_PSI] ;
