...
#define Kpsi_OH       (va[(20)])
#define Kpsi_H        (va[(21)])

#define Kpsi_Ca       (va[(22)])
#define Kpsi_CaOH     (va[(23)])

#define Kpsi_H2SiO4   (va[(24)])
#define Kpsi_H3SiO4   (va[(25)])

#define Kpsi_HSO4     (va[(26)])
#define Kpsi_SO4      (va[(27)])

#define Kpsi_K        (va[(28)])

#define Kpsi_Cl       (va[(29)])

#define Kpsi_Al       (va[(30)])
#define Kpsi_AlO4H4   (va[(31)])

#define Kpsi_CaH3SiO4 (va[(32)])

#define Kpsi_CaHSO4   (va[(33)])
...
#define V_AFt      (0) // (710.32e-3)      /* Thermochimie (ANDRA) */
...
static double r_afm,r_aft,r_c3ah6 ;
static double n_ca_ref,n_si_ref,n_al_ref ;
static double n_afm_0,n_aft_0,n_c3ah6_0 ;
...
#define I_N_CHn        (NEQ+37)
#define I_N_CSH2n      (NEQ+38)
#define I_N_AH3n       (NEQ+39)
#define I_N_AFmn       (NEQ+40)
#define I_N_AFtn       (NEQ+41)
#define I_N_C3AH6n     (NEQ+42)

#define I_N_Si_S       (NEQ+43)
#define I_N_Ca_S       (NEQ+44)
#define I_N_S_S        (NEQ+45)
#define I_N_Al_S       (NEQ+46)



#define I_S_CH         (NEQ+47)
#define I_S_SH         (NEQ+48)
#define I_S_CSH2       (NEQ+49)
#define I_S_AH3        (NEQ+50)
#define I_S_AFm        (NEQ+51)
#define I_S_AFt        (NEQ+52)
#define I_S_C3AH6      (NEQ+53)



#define I_RHO_L        (NEQ+54)



#define I_V_Cem        (NEQ+55)

#define I_V_Cem0       (NEQ+56)



#define I_PHI          (NEQ+57)
#define I_PHI_C        (NEQ+58)

#define I_PHIn         (NEQ+59)
#define I_PHI_Cn       (NEQ+60)



#define I_X_CSH        (NEQ+61)
#define I_V_CSH        (NEQ+62)


#define I_D_CON        (NEQ+63)

#define I_D_CONn       (NEQ+64)

#define I_P_CSH2       (NEQ+65)

#define I_C_OHn        (NEQ+66)
...
int pm(const char *s)
{
  ...
  else if(strcmp(s,"T_AFm") == 0)      return (9) ;
  else if(strcmp(s,"T_AFt") == 0)      return (10) ;
  else if(strcmp(s,"R_AFm") == 0)      return (11) ;
  else if(strcmp(s,"R_AFt") == 0)      return (12) ;
  else if(strcmp(s,"R_C3AH6") == 0)    return (13) ;
  else return(-1) ;
}
  ...
  */
  ...
  for(i = 0 ; i < nn ; i++) {
  ...
  
    {
      ...
      double v_cem      = V_CH*n_ch + v_csh*n_si_s + V_AH3*n_ah3 + V_AFm*n_afm + V_AFt*n_aft + V_C3AH6*n_c3ah6  ;
      /*
...
/* Termes explicites (va)  */
{
  ...
  */
  for(i = 0 ; i < 2 ; i++) {
    ...
    Kpsi_Al       += FsRT*d_al*iff*z_al*c_al ;
    Kpsi_AlO4H4   += FsRT*d_alo4h4*iff*z_alo4h4*c_alo4h4 ;
  ...
  
  {
    ...
    Kpsi_q         = z_h*Kpsi_H + z_oh*Kpsi_OH \
                   ...
                   + z_al*Kpsi_Al + z_alo4h4*Kpsi_AlO4H4\
...
double* ComputeFluxes(Element_t* el,double* grd,int i,int j)
{
  ...
  double grd_alo4h4   = grd[I_C_AlO4H4] ;
  ...
  double w_alo4h4   = - KF_AlO4H4*grd_alo4h4      - Kpsi_AlO4H4*grd_psi ;
...
void  ComputeSecondaryComponents(Element_t* el,double dt,double* x)
{
  ...
  */
  ...
  double c_alo4h4 = HardenedCementChemistry_GetAqueousConcentrationOf(hcc,AlO4H4) ;
  ...
  double n_si_s     = SiliconContentInCSH(zn_si_s) ;
  double n_ca_s     = n_ch + n_csh2 + x_csh*n_si_s + 4*n_afm + 6*n_aft + 3*n_c3ah6 ;
