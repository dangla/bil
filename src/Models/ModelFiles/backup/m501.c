...
/* Parametres */
static double phii,k_int,a_1,a_2,c_2,n_caoh20,n_csh0,f_jen0,f_tobI0,f_tobII0,x_na0,x_k0,a_cin,n_caco30 ;
...
int pm(char *s)
{
  ...
  else if(strcmp(s,"A_CIN") == 0) 	    return (15) ;
  else if(strcmp(s,"N_CaCO3") == 0) 	    return (16) ;
  else if(strcmp(s,"courbes") == 0) return (17) ;
  else return(-1) ;
}

int dm501(int dim,mate_t *mat,FILE *ficd)
...
void tb501(elem_t *el,inte_t *fi,unsigned int *n_fi,int dim)
{
  el->n_vi = NVI ;
...
void ch501(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,double *r,elem_t el,int dim,geom_t geom,double dt,double t,char_t cg)
...
void in501(double **x,double **u,double *f,double *va,elem_t el,int dim,geom_t geom)
/* Initialise les variables du systeme (f,va) */ 
{
  ...
  */
  ...
  a_cin   = el.mat->pr[pm("A_CIN")] ;
  n_caco30   = el.mat->pr[pm("N_CaCO3")] ;
  ...
  for(i=0;i<2;i++) {
    ...
    /* porosite */
     phi = phii + v_caoh2*(n_caoh20 - n_caoh2) + v_jen*(n_jen0 - n_jen) + v_tobII*(n_tobII0 - n_tobII) + v_tobI*(n_tobI0 - n_tobI) + v_sil*(n_sil0 - n_sil) + v_caco3*(n_caco30-n_caco3) ;
...
int ex501(double **x,double **u,double *f,double *va,elem_t el,int dim,geom_t geom,double t) 
...
int ct501(double **x,double **u,double **u_n,double *f,double *f_n,double *va,elem_t el,int dim,geom_t geom,double dt,double t)
/* Les variables donnees par la loi de comportement (f_1) */
{
  ...
  */
  ...
  for(i=0;i<el.nn;i++) {
    ...
    double n_co2,n_h2co3,n_hco3,n_co3 ;
    double n_oh,n_h,n_h2o ;
    double n_ca,n_caoh2,n_csh,n_jen,n_tobII,n_tobI,n_sil,n_caco3 ;
    double n_na,n_naoh,n_nahco3,n_naco3 ;
    double n_k,n_koh ;
    double n_h3sio4,n_h2sio4,n_h4sio4 ;
    double n_caoh,n_cahco3,n_caco3aq,n_caoh2aq;
...

    {
      ...
      double dn_caoh2sdt = dn1sdt*log(x_ca*x_oh*x_oh/k_2) ;
      double dn_caco3sdt = a_cin*(x_co3*x_ca/k_ca - 1) ;    
     ...
     /*double dn_caoh2sdt = - dn1sdt*(pow(fabs((k_ca*x_oh/(k_co3*k_2*x_hco3)-1)),Xp)) ; */
      ...
      n_caco3 = N_CaCO3n(i) + dt*dn_caco3sdt ;
...
x_naco3 = %e\n",x[i][0],x_co2,x_oh,x_h2o,x_hco3,n_caco3,x_na,x_k,x_naoh,x_nahco3,x_naco3) ;
    ...
    double phi = phii + v_caoh2*(n_caoh20 - n_caoh2) + v_jen*(n_jen0 - n_jen) + v_tobII*(n_tobII0 - n_tobII) + v_tobI*(n_tobI0 - n_tobI) + v_sil*(n_sil0 - n_sil) + v_caco3*(n_caco30-n_caco3) ;;
...
int mx501(double **x,double **u,double **u_n,double *f,double *f_n,double *va,double *k,elem_t el,int dim,geom_t geom,double dt,double t)
/* Matrice (k) */
{
#define K(i,j)    (k[(i)*2*NEQ+(j)])
  ...
  double dphisdx_oh,dphisdx_hco3,dphisdx_h3sio4,dphisdx_ca,dphisdn_csh ;

  double ds_lsdp_l,ds_lsdp_c ;
  
  double dx_h2co3sdx_co2[2] ;
  double dx_hsdx_oh[2],dx_co3sdx_oh[2],dx_co3sdx_hco3[2] ;

  double dx_h2osdx_co2,dx_h2osdx_hco3,dx_h2osdx_oh,dx_h2osdx_na,dx_h2osdx_k,dx_h2osdx_h3sio4,dx_h2osdx_ca  ;
  ...
  double dx_caohsdx_ca[2],dx_caohsdx_oh[2] ;
  double dx_cahco3sdx_ca[2],dx_cahco3sdx_hco3[2] ;
  double dx_caoh2aqsdx_oh[2],dx_caoh2aqsdx_ca[2] ;

  double dx_h2sio4sdx_h3sio4[2],dx_h4sio4sdx_h3sio4[2],dx_h2sio4sdx_oh[2],dx_h4sio4sdx_oh[2] ;

  double dx_cah3sio4sdx_ca[2],dx_cah3sio4sdx_h3sio4[2],dx_cah2sio4sdx_h3sio4[2],dx_cah2sio4sdx_oh[2],dx_cah2sio4sdx_ca[2];

  double dn_caoh2sdx_oh,dn_caoh2sdx_ca ;
  double dn_caco3sdx_oh,dn_caco3sdx_ca,dn_caco3sdx_hco3 ;

  double dxisdx_oh,dxisdx_co2,dxisdx_hco3 ;

  double df_jensdx_h3sio4,df_jensdx_oh,df_jensdx_ca;
  double df_tobIIsdx_h3sio4,df_tobIIsdx_oh,df_tobIIsdx_ca;
  double df_tobIsdx_h3sio4,df_tobIsdx_oh,df_tobIsdx_ca;
  double df_silsdx_h3sio4,df_silsdx_oh;

  double drho_lsdx_oh,drho_lsdx_hco3,drho_lsdx_co2,drho_lsdx_na,drho_lsdx_k,drho_lsdx_h3sio4,drho_lsdx_ca ;
  ...
  */
  for(i=0;i<2;i++) {
    ...
    dx_caohsdx_oh[i] = x_caoh/x_oh ;
    dx_caohsdx_ca[i] = x_caoh/x_ca ;
    
    dx_cahco3sdx_ca[i] =  x_cahco3/x_ca ;
    dx_cahco3sdx_hco3[i] =  x_cahco3/x_hco3 ;

    dx_caoh2aqsdx_oh[i]= 2*x_caoh2aq/x_oh ;
    dx_caoh2aqsdx_ca[i]= x_caoh2aq/x_ca ;
    ...
    dx_cah2sio4sdx_ca[i]   =  x_cah2sio4/x_ca ;
    dx_cah2sio4sdx_h3sio4[i] =   x_cah2sio4/x_h3sio4 ;
    dx_cah2sio4sdx_oh[i] =   x_cah2sio4/x_oh ;

    dx_cah3sio4sdx_ca[i]     =  x_cah3sio4/x_ca ;
    dx_cah3sio4sdx_h3sio4[i]   = x_cah3sio4/x_h3sio4 ;

    df_jensdx_h3sio4   =  y_jen*f_jen/x_h3sio4 ;
    df_jensdx_oh       =  (2*x_jen - y_jen)*f_jen/x_oh ;
    df_jensdx_ca   =  x_jen*f_jen/x_ca ;

    df_tobIIsdx_h3sio4   =  y_tobII*f_tobII/x_h3sio4 ;
    df_tobIIsdx_oh       =  (2*x_tobII - y_tobII)*f_tobII/x_oh ;
    df_tobIIsdx_ca   =  x_tobII*f_tobII/x_ca ;
    
    df_tobIsdx_h3sio4   =  y_tobI*f_tobI/x_h3sio4 ;
    df_tobIsdx_oh       =  (2*x_tobI - y_tobI)*f_tobI/x_oh ;
    df_tobIsdx_ca   =  x_tobI*f_tobI/x_ca ;

    df_silsdx_h3sio4   =  f_sil/x_h3sio4 ;
    df_silsdx_oh       =  - f_sil/x_oh ;

    dx_h2osdx_co2      = -(v_h2co3*dx_h2co3sdx_co2[i])/v_h2o ;
    dx_h2osdx_hco3     = -(v_hco3 + v_co3*dx_co3sdx_hco3[i] + v_nahco3*dx_nahco3sdx_hco3[i] + v_naco3*dx_naco3sdx_hco3[i] +  v_cahco3*dx_cahco3sdx_hco3[i])/v_h2o ;
    dx_h2osdx_oh       = -(v_co3*dx_co3sdx_oh[i] + v_oh + v_h*dx_hsdx_oh[i] + v_naoh*dx_naohsdx_oh[i] + v_naco3*dx_naco3sdx_oh[i] + v_koh*dx_kohsdx_oh[i] + v_caoh*dx_caohsdx_oh[i] + v_caoh2aq*dx_caoh2aqsdx_oh[i] + v_h2sio4*dx_h2sio4sdx_oh[i] + v_h4sio4*dx_h4sio4sdx_oh[i] + v_cah2sio4*dx_cah2sio4sdx_oh[i] )/v_h2o ;
    ...
    dx_h2osdx_ca     = -(v_ca + v_caoh*dx_caohsdx_ca[i] +  v_cahco3*dx_cahco3sdx_ca[i] + v_caoh2aq*dx_caoh2aqsdx_ca[i] + v_cah2sio4*dx_cah2sio4sdx_ca[i] + v_cah3sio4*dx_cah3sio4sdx_ca[i])/v_h2o ;

    drho_lsdx_co2      = M_H2CO3*dx_h2co3sdx_co2[i] + M_H2O*dx_h2osdx_co2 ;
    drho_lsdx_oh       = M_H*dx_hsdx_oh[i] + M_OH + M_H2O*dx_h2osdx_oh + M_CO3*dx_co3sdx_oh[i] + M_NaOH*dx_naohsdx_oh[i] + M_NaCO3*dx_naco3sdx_oh[i] + M_KOH*dx_kohsdx_oh[i] + M_CaOH2aq*dx_caoh2aqsdx_oh[i] + M_CaOH*dx_caohsdx_oh[i] + M_H2SiO4*dx_h2sio4sdx_oh[i] + M_H4SiO4*dx_h4sio4sdx_oh[i] + M_CaH2SiO4*dx_cah2sio4sdx_oh[i]  ;
    drho_lsdx_hco3     = M_H2O*dx_h2osdx_hco3 + M_HCO3 + M_CO3*dx_co3sdx_hco3[i] + M_NaHCO3*dx_nahco3sdx_hco3[i] + M_NaCO3*dx_naco3sdx_hco3[i] + M_CaHCO3*dx_cahco3sdx_hco3[i] ;
    ...
    drho_lsdx_ca     = M_H2O*dx_h2osdx_ca + M_Ca + M_CaOH*dx_caohsdx_ca[i] + M_CaHCO3*dx_cahco3sdx_ca[i] + M_CaOH2aq*dx_caoh2aqsdx_ca[i] + M_CaH2SiO4*dx_cah2sio4sdx_ca[i] + M_CaH3SiO4*dx_cah3sio4sdx_ca[i] ;
    ...
    if(n_caoh2 > 0.) {
      ...
      /*cinétique de dissolution de la portlandite en loi log*/
      dn_caoh2sdx_oh    =  2*dn1/x_oh ;
      dn_caoh2sdx_ca  = dn1/x_ca ;
      /*cinétique de dissolution de la portlandite en loi puissance*/
     /* dn_caoh2sdx_oh    =  Xp*dn1*k_ca/(k_co3*k_2*x_hco3)*(pow(fabs((k_ca*x_oh/(k_co3*k_2*x_hco3)-1)),(Xp-1))) ;
      ...
      dn_caco3sdx_oh    =  a_cin*(k_co3*x_hco3*x_ca/k_ca) ;
      dn_caco3sdx_ca  = a_cin*(k_co3*x_hco3*x_oh/k_ca);
      dn_caco3sdx_hco3  = a_cin*(k_co3*x_oh*x_ca/k_ca) ;

      /*dn_caco3sdx_oh    =  a_cin/x_oh ;
      dn_caco3sdx_ca  = a_cin/x_ca ;
      dn_caco3sdx_hco3  = a_cin/x_hco3 ;*/

    dphisdx_oh         = - v_caoh2*dn_caoh2sdx_oh - v_jen*n_csh*df_jensdx_oh - v_tobII*n_csh*df_tobIIsdx_oh - v_tobI*n_csh*df_tobIsdx_oh - v_sil*n_csh*df_silsdx_oh - v_caco3*dn_caco3sdx_oh  ;
    dphisdx_hco3       = - v_caco3*dn_caco3sdx_hco3   ;
    ...
    dphisdx_ca         = - v_caoh2*dn_caoh2sdx_ca - v_jen*n_csh*df_jensdx_ca - v_tobII*n_csh*df_tobIIsdx_ca - v_tobI*n_csh*df_tobIsdx_ca - v_caco3*dn_caco3sdx_ca ;
    ...
    */
    ...
    K(E_C+j,I_OH+j)    += volume[i]*(phi*s_l*(dx_co3sdx_oh[i] + dx_naco3sdx_oh[i])  + dphisdx_oh*(s_g*x_co2 + s_l*(x_h2co3 + x_hco3 + x_co3 + x_nahco3 + x_naco3 + x_cahco3 + x_caco3aq)) + dn_caco3sdx_oh) ;

    K(E_C+j,I_HCO3+j)  += volume[i]*(phi*s_l*(un + dx_co3sdx_hco3[i] + dx_naco3sdx_hco3[i] + dx_nahco3sdx_hco3[i] + dx_cahco3sdx_hco3[i]) + dphisdx_hco3*(s_g*x_co2 + s_l*(x_h2co3 + x_hco3 + x_co3 + x_nahco3 + x_naco3 + x_cahco3 + x_caco3aq )) + dn_caco3sdx_hco3) ;

    K(E_C+j,I_Ca+j) += volume[i]*(dphisdx_ca*(s_g*x_co2 + s_l*(x_h2co3 + x_hco3 + x_co3 + x_nahco3 + x_naco3 + x_caco3aq)) + dn_caco3sdx_ca)  ;
    ...
    */
    ...
    K(E_mass+j,I_OH+j)    += volume[i]*(phi*s_l*drho_lsdx_oh + dphisdx_oh*(s_l*rho_l + M_CO2*s_g*x_co2) + M_CaOH2*dn_caoh2sdx_oh + M_jen*df_jensdx_oh*n_csh + M_tobII*df_tobIIsdx_oh*n_csh + M_tobI*df_tobIsdx_oh*n_csh + M_sil*df_silsdx_oh*n_csh + M_CaCO3*dn_caco3sdx_oh) ;

    K(E_mass+j,I_HCO3+j)  += volume[i]*(phi*s_l*drho_lsdx_hco3 + dphisdx_hco3*(s_l*rho_l + M_CO2*s_g*x_co2) + M_CaCO3*dn_caco3sdx_hco3) ;

    K(E_mass+j,I_Ca+j)    += volume[i]*(phi*s_l*drho_lsdx_ca + dphisdx_ca*(s_l*rho_l + M_CO2*s_g*x_co2) + M_CaOH2*dn_caoh2sdx_ca + M_jen*df_jensdx_ca*n_csh + M_tobII*df_tobIIsdx_ca*n_csh + M_tobI*df_tobIsdx_ca*n_csh + M_CaCO3*dn_caco3sdx_ca) ;
    ...
    */
    ...
    K(E_Ca+j,I_OH+j)    += volume[i]*(phi*s_l*(dx_caoh2aqsdx_oh[i] + dx_caohsdx_oh[i] + dx_cah2sio4sdx_oh[i]) + dn_caoh2sdx_oh + dn_caco3sdx_oh + x_jen*df_jensdx_oh*n_csh + x_tobII*df_tobIIsdx_oh*n_csh + x_tobI*df_tobIsdx_oh*n_csh + dphisdx_oh*s_l*(x_ca + x_caoh + x_cahco3 + x_caco3aq + x_caoh2aq + x_cah2sio4 + x_cah3sio4)) ;

    K(E_Ca+j,I_HCO3+j)  += volume[i]*(phi*s_l*(dx_cahco3sdx_hco3[i]) + dn_caco3sdx_hco3 + dphisdx_hco3*s_l*(x_ca + x_caoh + x_cahco3 + x_caco3aq + x_caoh2aq + x_cah2sio4 + x_cah3sio4)) ;

    K(E_Ca+j,I_Ca+j) += volume[i]*(phi*s_l*(un + dx_caoh2aqsdx_ca[i] + dx_caohsdx_ca[i] + dx_cah2sio4sdx_ca[i] + dx_cah3sio4sdx_ca[i] + dx_cahco3sdx_ca[i]   ) + dn_caoh2sdx_ca + dn_caco3sdx_ca + x_jen*df_jensdx_ca*n_csh + x_tobII*df_tobIIsdx_ca*n_csh + x_tobI*df_tobIsdx_ca*n_csh + dphisdx_ca*s_l*(x_ca + x_caoh + x_cahco3 + x_caco3aq + x_caoh2aq + x_cah2sio4 + x_cah3sio4)) ;
    ...
    */
    ...
    K(E_k+j,I_OH+j)    += volume[i]*(phi*s_l*(dx_co3sdx_oh[i] + dx_naco3sdx_oh[i]) + dphisdx_oh*s_l*(x_co3 + x_hco3 + x_nahco3 + x_naco3 + x_caco3aq + x_cahco3) + dn_caco3sdx_oh - dphisdx_oh*pow(s_l,Xcinetique)*dt*xi - phi*pow(s_l,Xcinetique)*dt*dxisdx_oh );

    K(E_k+j,I_HCO3+j)  += volume[i]*(phi*s_l*(un + dx_co3sdx_hco3[i] + dx_nahco3sdx_hco3[i] + dx_naco3sdx_hco3[i]  + dx_cahco3sdx_hco3[i]) + dphisdx_hco3*s_l*(x_co3 + x_hco3 + x_nahco3 + x_naco3 + x_caco3aq + x_cahco3 ) + dn_caco3sdx_hco3 - dphisdx_hco3*pow(s_l,Xcinetique)*dt*xi - phi*pow(s_l,Xcinetique)*dt*dxisdx_hco3 );

    K(E_k+j,I_Ca+j)  += volume[i]*(phi*s_l*(dx_cahco3sdx_ca[i]) + dphisdx_ca*s_l*(x_co3 + x_hco3 + x_nahco3 + x_naco3 + x_caco3aq + x_cahco3 ) + dn_caco3sdx_ca - dphisdx_ca*pow(s_l,Xcinetique)*dt*xi);
    ...
    */
    K(E_el+j,I_OH+j)    += volume[i]*(z_oh + z_h*dx_hsdx_oh[i] + z_co3*dx_co3sdx_oh[i] + z_naco3*dx_naco3sdx_oh[i] + z_h2sio4*dx_h2sio4sdx_oh[i] + z_caoh*dx_caohsdx_oh[i]) ;

    K(E_el+j,I_HCO3+j)  += volume[i]*(z_hco3 + z_co3*dx_co3sdx_hco3[i] + z_naco3*dx_naco3sdx_hco3[i] + z_cahco3*dx_cahco3sdx_hco3[i]) ;

    K(E_el+j,I_Ca+j)    += volume[i]*(z_ca + z_caoh*dx_caohsdx_ca[i] + z_cahco3*dx_cahco3sdx_ca[i] + z_cah3sio4*dx_cah3sio4sdx_ca[i]) ;
    ...
    */
    ...
    K(E_Na+j,I_Ca+j) += volume[i]*dphisdx_ca*s_l*(x_na + x_naoh + x_nahco3 + x_naco3 );
    ...
    */
    ...
    K(E_K+j,I_Ca+j)  += volume[i]*dphisdx_ca*s_l*(x_k + x_koh) ;
    ...
    */
    ...
    K(E_Si+j,I_OH+j)       += volume[i]*(phi*s_l*(dx_h2sio4sdx_oh[i] + dx_h4sio4sdx_oh[i] + dx_cah2sio4sdx_oh[i]) + dphisdx_oh*s_l*(x_h3sio4 + x_h2sio4 + x_h4sio4 + x_cah2sio4 + x_cah3sio4) + y_jen*df_jensdx_oh*n_csh + y_tobII*df_tobIIsdx_oh*n_csh + y_tobI*df_tobIsdx_oh*n_csh + y_sil*df_silsdx_oh*n_csh) ;

    K(E_Si+j,I_HCO3+j)     += volume[i]*(dphisdx_hco3*s_l*(x_h3sio4 + x_h2sio4 + x_h4sio4 + x_cah2sio4 + x_cah3sio4)) ;

    K(E_Si+j,I_Ca+j)    += volume[i]*(phi*s_l*(dx_cah2sio4sdx_ca[i] + dx_cah3sio4sdx_ca[i]) + dphisdx_ca*s_l*(x_h3sio4 + x_h2sio4 + x_h4sio4 + x_cah2sio4 + x_cah3sio4) + y_jen*df_jensdx_ca*n_csh + y_tobII*df_tobIIsdx_ca*n_csh + y_tobI*df_tobIsdx_ca*n_csh) ;
    ...
    */
    ...
    K(E_f+j,I_Ca+j)  += volume[i]*(- df_jensdx_ca - df_tobIIsdx_ca - df_tobIsdx_ca) ;
  ...
  */
  ...
  for(i=0;i<2;i++){
    c[i] = trf_hco3 + trf_co3*dx_co3sdx_hco3[i] + trf_nahco3*dx_nahco3sdx_hco3[i] + trf_naco3*dx_naco3sdx_hco3[i] + trf_cahco3*dx_cahco3sdx_hco3[i]  ;
  }
  K(E_C,I_HCO3)         += + c[0] ;
  ...
  for(i=0;i<2;i++){
    c[i] = trf_co3*dx_co3sdx_oh[i] + trf_naco3*dx_naco3sdx_oh[i] ;
  }
  K(E_C,I_OH)           += + c[0] ;
  K(E_C,I_OH+NEQ)       += - c[1] ;
  K(E_C+NEQ,I_OH)       += - c[0] ;
  ...
  for(i=0;i<2;i++){
  c[i] =trf_cahco3*dx_cahco3sdx_ca[i]  ;
  }
  K(E_C,I_Ca)           += + c[0] ;
  K(E_C,I_Ca+NEQ)       += - c[1] ;
  K(E_C+NEQ,I_Ca)       += - c[0] ;
  K(E_C+NEQ,I_Ca+NEQ)   += + c[1] ;

  /*
    Conservation de la charge  : div(w_q) = 0
  */
  for(i=0;i<2;i++){
    c[i] = z_hco3*trf_hco3 + z_co3*trf_co3*dx_co3sdx_hco3[i] + z_naco3*trf_naco3*dx_naco3sdx_hco3[i] + z_cahco3*trf_cahco3*dx_cahco3sdx_hco3[i] ;
  }
  K(E_q,I_HCO3)         += + c[0] ;
  ...
  for(i=0;i<2;i++){
    c[i] = z_h*trf_h*dx_hsdx_oh[i] + z_oh*trf_oh + z_co3*trf_co3*dx_co3sdx_oh[i] + z_naco3*trf_naco3*dx_naco3sdx_oh[i] + z_caoh*trf_caoh*dx_caohsdx_oh[i] +z_h2sio4*trf_h2sio4*dx_h2sio4sdx_oh[i]  ;
  }
  K(E_q,I_OH)           += + c[0] ;
  K(E_q,I_OH+NEQ)       += - c[1] ;
  K(E_q+NEQ,I_OH)       += - c[0] ;
  ...
  for(i=0;i<2;i++){
    c[i] = z_ca*trf_ca + z_caoh*trf_caoh*dx_caohsdx_ca[i] + z_cahco3*trf_cahco3*dx_cahco3sdx_ca[i] + z_cah3sio4*trf_cah3sio4*dx_cah3sio4sdx_ca[i]  ;
  }
  K(E_q,I_Ca)           += + c[0] ;
  K(E_q,I_Ca+NEQ)       += - c[1] ;
  K(E_q+NEQ,I_Ca)       += - c[0] ;
  K(E_q+NEQ,I_Ca+NEQ)   += + c[1] ;
  ...
  */
  ...
  for(i=0;i<2;i++){
    c[i] = trf_cahco3*dx_cahco3sdx_hco3[i] ;
  }
  K(E_Ca,I_HCO3)         += + c[0] ;
  ...
  for(i=0;i<2;i++){
    c[i] = trf_caoh2aq*dx_caoh2aqsdx_oh[i] + trf_caoh*dx_caohsdx_oh[i] +  trf_cah2sio4*dx_cah2sio4sdx_oh[i];
  }
  K(E_Ca,I_OH)           += + c[0] ;
  K(E_Ca,I_OH+NEQ)       += - c[1] ;
  K(E_Ca+NEQ,I_OH)       += - c[0] ;
  ...
  for(i=0;i<2;i++){
    c[i] = trf_ca + trf_caoh2aq*dx_caoh2aqsdx_ca[i] + trf_caoh*dx_caohsdx_ca[i] +  trf_cah2sio4*dx_cah2sio4sdx_ca[i] + trf_cah3sio4*dx_cah3sio4sdx_ca[i] + trf_cahco3*dx_cahco3sdx_ca[i];
  }
  K(E_Ca,I_Ca)           += + c[0] ;
  K(E_Ca,I_Ca+NEQ)       += - c[1] ;
  K(E_Ca+NEQ,I_Ca)       += - c[0] ;
  K(E_Ca+NEQ,I_Ca+NEQ)   += + c[1] ;
  ...
  */
  ...
  for(i=0;i<2;i++){
    c[i] = trf_hco3 + trf_co3*dx_co3sdx_hco3[i] + trd_nahco3*dx_nahco3sdx_hco3[i] + trd_naco3*dx_naco3sdx_hco3[i] + trd_cahco3*dx_cahco3sdx_hco3[i]   ;
  }
  K(E_k,I_HCO3)          += + c[0] ;
  ...
  for(i=0;i<2;i++){
    c[i] = trf_co3*dx_co3sdx_oh[i] + trd_naco3*dx_naco3sdx_oh[i] ;
  }
  K(E_k,I_OH)            += + c[0] ;
  K(E_k,I_OH+NEQ)        += - c[1] ;
  K(E_k+NEQ,I_OH)        += - c[0] ;
  ...
  for(i=0;i<2;i++){
    c[i] = trd_cahco3*dx_cahco3sdx_ca[i]   ;
  }
  K(E_k,I_Ca)          += + c[0] ;
  K(E_k,I_Ca+NEQ)      += - c[1] ;
  K(E_k+NEQ,I_Ca)      += - c[0] ;
  K(E_k+NEQ,I_Ca+NEQ)  += + c[1] ;
  ...
  */
  ...
  for(i=0;i<2;i++){
    c[i] = trf_h2sio4*dx_h2sio4sdx_oh[i] + trf_h4sio4*dx_h4sio4sdx_oh[i] + trf_cah2sio4*dx_cah2sio4sdx_oh[i];
  }
  K(E_Si,I_OH)           += + c[0] ;
  K(E_Si,I_OH+NEQ)       += - c[1] ;
  K(E_Si+NEQ,I_OH)       += - c[0] ;
  ...
  for(i=0;i<2;i++){
    c[i] = trf_cah2sio4*dx_cah2sio4sdx_ca[i] + trf_cah3sio4*dx_cah3sio4sdx_ca[i]  ;
  }
  K(E_Si,I_Ca)           += + c[0] ;
  K(E_Si,I_Ca+NEQ)       += - c[1] ;
  K(E_Si+NEQ,I_Ca)       += - c[0] ;
  K(E_Si+NEQ,I_Ca+NEQ)   += + c[1] ;
...
void rs501(double **x,double **u,double **u_n,double *f,double *f_n,double *va,double *r,elem_t el,int dim,geom_t geom,double dt,double t)
/* Residu (r) */
{
...
int so501(double **x,double **u,double *f,double *va,double *s,resu_t *r,elem_t el,int dim,geom_t geom,double t)
...
/* Termes explicites (va)  */
{
  ...
  */
  for(i=0;i<2;i++) {
    ...
    double n_tobII0 = f_tobII0*n_csh0 ;
    double n_sil0 = (1 - f_jen0 - f_tobI0 - f_tobII0)*n_csh0 ;
    double phi = phi = phii + v_caoh2*(n_caoh20 - n_caoh2) + v_jen*(n_jen0 - n_jen) + v_tobII*(n_tobII0 - n_tobII) + v_tobI*(n_tobI0 - n_tobI) + v_sil*(n_sil0 - n_sil) + v_caco3*(n_caco30-n_caco3) ; ;
...
/* Les flux (f) */
{
  ...
  for(i=0;i<2;i++) {
    ...
    x_caoh[i]   	= k_caoh/k_e*x_oh*x_ca ;
    x_cahco3[i] 	= k_cahco3*x_hco3*x_ca ;
    x_caco3aq[i] 	= k_caco3*k_ca/(k_e*k_co3) ;
    x_caoh2aq[i] 	= k_caoh2*x_ca*x_oh*x_oh ;
    x_h2sio4[i]  	= x_oh*x_h3sio4/(k_e*k_h2sio4) ;
    x_h4sio4[i] 	= (x_h3sio4/x_oh)*(k_e*k_h4sio4/k_h2sio4) ;
    x_cah2sio4[i] 	= k_cah2sio4*x_h2sio4[i]*x_ca;
    x_cah3sio4[i] 	= k_cah3sio4*x_h3sio4*x_ca;
  }
