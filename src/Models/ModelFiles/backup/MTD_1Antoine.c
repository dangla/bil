...
#define TITLE   "Atmospheric Carbonation Of CBM (2013)"
...
#define d_na       		(1.33e-5)
#define d_naoh     		(1.33e-5)
#define d_nahco3  	  (1.33e-5)
#define d_naco3    	  (1.33e-5)
#define d_k        		(1.957e-5)
#define d_koh      		(1.957e-5)
  ...
  */
  ...
  for(i = 0 ; i < nn ; i++) {
...

    {
      ...
      if(x_co2 < 0 || x_oh <= 0 || x_h2o <= 0 || x_ca < 0 || n_si_s < 0.) {
...
#define UNKNOWN_s(i)   UNKNOWN(j,i)
  ...
  */
  ...
  double coeff_dnCH = log((k_ca*x_oh/(k_co3*k_2*x_hco3)));
    ...
    /* permeabilite Verma Pruess 1988*/
	...
	double verma_pruess = t*t*( 1-frac+(frac/(w*w))  )/( 1 - frac + frac*(pow(t/(t+w-1),2.))   ) ;
    double coeff_permeability = verma_pruess ; /* Change here */
...
/* Termes explicites (va)  */
{
  ...
  */
  for(i = 0 ; i < 2 ; i++) {
    ...
    /* permeabilite Kozeni-Carman */
	double kozeni_carman = pow(phi/phii,3.)*pow(((1-phii)/(1-phi)),2.);
	...
	{
		t = (1-S_s-phi_r)/(1-phi_r) ;
		verma_pruess = t*t*( 1-frac+(frac/(w*w))  )/( 1 - frac + frac*(pow(t/(t+w-1),2.))   ) ;
	}
