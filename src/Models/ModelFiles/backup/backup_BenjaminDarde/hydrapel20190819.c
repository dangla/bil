  ...
  */
  {
    ...
    for(i = 0 ; i < nn ; i++) {
      ...
      for(j = 0 ; j < nn ; j++) {
        K(E_liq + i*NEQ,U_p_l2 + j*NEQ) += dt*kc[i*nn + j] ;
      }
    }
  }
...
// ---------------------------------------------       contraintes et deformations 
          ...
          double epsV = x[I_EPS + 0]+x[I_EPS + 4]+x[I_EPS + 8] ;
          double epsV1 = x[I_EPSV_1] ;
// ---------------------------------------------     
...
// ---------------------------------------------       propriétés du matériau 
          ...
          double solid_fraction_1         = initial_solid_fraction_1 * (1+epsV1)/(1+epsV) ;
          double solid_fraction_2         = initial_solid_fraction_2 * (1+epsV1)/(1+epsV) ;
          double bulk ;
// ---------------------------------------------     

        if(isGran == true) {
             double m       = calcul_m(sigm,solid_fraction_1,p_l1); // " d_rond epsilon_V / d_rond m "
...
// ---------------------------------------------     
        ...
        if(isGran == true) {
             ...
             double f_Mm    = 1. ; // " d_rond epsilon_V / d_rond epsilon_V1 "
             double deps1dp = calcul_deps1dp(p_l1,sigm,solid_fraction_1) ; // "d_rond epsilon_V1 / d_rond p "
             double dmds    = calcul_dmds(p_l1,sigm,solid_fraction_1) ; 
             double deps1ds = calcul_deps1ds(p_l1,sigm,solid_fraction_1) ; 

             bulk  =  dFdm * dmdp + f_Mm * deps1dp;
             bulk  =  1./bulk;
             biot1 =  dFdm * dmds + f_Mm * deps1ds ;
             biot1 =  biot1 * bulk ;
        } else {
             double propPel  = p_pellet / (p_pellet + p_poudre) ;
             ...
             biot1 = (-propPel*kappa_m) / sigmeff1 / kappa_eq ;
        }

        for(i = 0 ; i < 3 ; i++) B1(i,i) = biot1 ;
...
// ---------------------------------------------     
        ...
        if(isGran == true) {
             ...
             double sigm2   = sigm / (solid_fraction_1 + solid_fraction_2) ;
             double sigmeff2 = sigm2 + p_l2 ; // p2
             double kappa_eq = calcul_kappa_eq(propPel,sigm,p_l1,p_l2,solid_fraction_1,solid_fraction_2) ;

             biot2 = (-propPou*kappa_m) / sigmeff2 / kappa_eq - kappa_s / (-s2) / kappa_eq ;
        }
    ...
    #if 0
    {
      ...
      /* Storage matrix */
      {
        ...
        double phi      = 1. - solid_fraction_1 - solid_fraction_2 ; // porosite macro = VvM/V = (V - V1 - V2)/V = 1 - sol.frac.1 - sol.frac.2
...
/* calcul temps caractéristique de diffusion */
          ...
          double delta_p_l_n = p_l2n - p_l1n ; // > 0 si s1 > s2
          double C_diff_n = k_int / (mu_l * deps1ds_n) ; // > 0
          double tau_hydro_n = initial_equivalent_radius * initial_equivalent_radius * (pow(eps_1_n,2./3.)) / C_diff_n ; // temps caractéristique de diffusion au temps t_n

/* calcul beta, paramètre de transfert micro-macro */
          double beta_hydro_n = beta_hydro_0*exp(- beta_hydro_a*(-p_l1n)) ;

/* calcul masse echangee (kg pour un m3 de "pellets") */
          double vol_pel = 4./3. * M_PI * pow(initial_equivalent_radius,3.) ;
          double delta_m_w_1 = ( dt * beta_hydro_n / tau_hydro_n * delta_p_l_n ) / ( vol_pel ) ;
          delta_m_w_1 = delta_m_w_1 * solid_fraction_1 ; // masses sont exprimees en kg pour un m3 de materiau

/* calcul succion micro */

// delta_deformation_vol_pellets = delta_masse_w_pellets / (rho_l * initial_solid_fraction_1/(1+epsV))
// delta_deformation_vol_pellets = delta_deformation_vol_pellets(sigm,p_l1)

          double sigm_1_n   = sigm_n / solid_fraction_1 + p_l1n ;
          double initial_e1 = rho_solid / initial_pel_dry_dens - 1. ;  ;

          double exp_alpha_sigmn = exp(- alpha * (- sigm_1_n) ) ;
          double sigm_1_estim    = log(alpha / beta * delta_m_w_1 / rho_l0 / (initial_solid_fraction_1/(1.+tr_eps_n)) + exp_alpha_sigmn) / ( - alpha ) ;
          sigm_1_estim           = - sigm_1_estim ;

          double p_l1_estim = sigm_1_estim - sigm_n / solid_fraction_1 ;
          
          double p_l1 = p_l1_estim ;
...
//  XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
    ...
    /* Total liquid mass content and body force */
    {
      ...
      double  sat_vap_M = 1. - sat_liq_M ; //saturation vapeur eau dans les macropores.
      double  rel_hum_M = exp(M_R_T/rho_l * p_l2) ;
      double  p_vap_M = p_vap_sat * p_vap_sat ;
      double  rho_vap = M_R_T * p_vap_M ;

      double  m_l_M  = rho_l * phi_M * sat_liq_M + rho_vap * phi_M * sat_vap_M ; // dans les macropores ;
      double  m_l_1  = rho_l * phi_m1 * solid_fraction_1 ; // dans les grains de poudre, revient à rho_l * (initial_poro + eps1)/(1+eps1) * initial_sol_frac * (1+eps1) / (1+epsV) = rho_l * (n_0 + eps1) * phi_1_0 / (1+tr_eps)
      double  m_l_2  = rho_l * phi_m2 * solid_fraction_2 ; // dans les pellets 

      double  m_l    = m_l_1 + m_l_2 + m_l_M ;
      ...
      x[I_M_L]   = m_l ;
      x[I_RHO_L] = rho_l ;
      x[I_PHI]   = phi ; // porosite macro ici
    ...
    {
      double m_l_1 = rho_l * phi_m1 * solid_fraction_1 ; // m_w = rho * n * Phi [kg pour un m3 de matériau]  =  rho  *  (eps1 + n0)/(1+eps1)  *  Phi  
      x[I_M_L1]    = m_l_1 ;
    }
  }
  #endif
