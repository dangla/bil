...
#define CSRn(n)       (f_n[(8+n)])
#define Q_CH_EQn(n)   (f_n[(14+n)])
...
/* C/S ratio */
#define X_CSH(q)         (Curve_ComputeValue(Element_GetCurve(el) + 6,q))
#define DX_CSH(q)        (Curve_ComputeDerivative(Element_GetCurve(el) + 6,q))
/*
...
*/
#define Z_CSH(x)         (Curve_ComputeValue(Element_GetCurve(el) + 3,x))
#define DZ_CSH(x)        (Curve_ComputeDerivative(Element_GetCurve(el) + 3,x))
#define V_CSH(x)         (Curve_ComputeValue(Element_GetCurve(el) + 4,x))
#define DV_CSH(x)        (Curve_ComputeDerivative(Element_GetCurve(el) + 4,x))

#define DGSDX(x)         (Curve_ComputeValue(Element_GetCurve(el) + 2,x))
#define D2GSDX2(x)       (Curve_ComputeDerivative(Element_GetCurve(el) + 2,x))
#define LNK_CSH(x)       (Curve_ComputeValue(Element_GetCurve(el) + 5,x))

/* Ion Activity Products */
#define X_CO2_eq         (k_ca*k_1/(k_co3*k_h*k_2))
#define Q_CH(x_co2)      (X_CO2_eq/x_co2)
#define LNQ_SH(q,x)      (LNK_CSH(x) - x*log(q))
#define Q_SH(q,x)        (exp(LNQ_SH(q,x)))
#define IAP_SH(q,x)      (k_sil*Q_SH(q,x))
...
static double* ComputeAqueousVariables(Element_t*,double,double**,int) ;
...
static double ComputeCSValue(Element_t*,double,double,double) ;
static double ComputeCSDerivative(Element_t*,double,double,double) ;
static double ComputeQ_CH_EQ(Element_t*,double,double,double) ;
static double ComputeDQ_CH_EQ(Element_t*,double,double,double) ;
static double ComputeQ_CH_EQ1(Element_t*,double,double,double) ;
...
#define NbOfAqueousVariables    (37)
...
/* Initialise les variables du systeme (f,va) */ 
{
  ...
  */
  ...
  for(i = 0 ; i < nn ; i++) {
    ...
    double q_ch       = Q_CH(x_co2) ;
    double q_ch_eq    = ComputeQ_CH_EQ(el,0,q_ch,1) ;
    double x_csh      = X_CSH(q_ch_eq) ;
    double *x = ComputeAqueousVariables(el,x_csh,u,i) ;
    ...
    /* porosite */
    double v_csh    = V_CSH(x_csh) ;
    double v_csh0   = V_CSH(x_csh) ;
...
/* Les variables donnees par la loi de comportement (f_1) */
{
  ...
  */
  ...
  for(i = 0 ; i < nn ; i++) {
    ...
    double x_co2      = X_CO2(i) ;
    double q_ch       = Q_CH(x_co2) ;
    double tausdt     = GetProperty("Tau")/dt ;
    double q_ch_eq    = ComputeQ_CH_EQ(el,tausdt,q_ch,Q_CH_EQn(i)) ;
    double x_csh      = X_CSH(q_ch_eq) ;
    ...
    /* porosite */
    double x_csh0   = CSR0(i) ;
    double v_csh    = V_CSH(x_csh) ;
    double v_csh0   = V_CSH(x_csh0) ;
    double phi = phii + V_CH*(n_ch0 - n_ch) + v_csh0*n_si_s0 - v_csh*n_si_s + V_CC*(-n_cc) ;
    double phi_l = phi*s_l ;
    double phi_g = phi*s_g ;
...
#define K(i,j)    (k[(i)*2*NEQ+(j)])
  ...
  double Dx_hco3SDx_co2[2] ;
  double Dx_co3SDx_co2[2] ;
  double Dx_caSDx_co2[2] ;
  double Dx_caohSDx_co2[2] ;
  double Dx_caoh2aqSDx_co2[2] ;
  double Dx_h2sio4SDx_co2[2] ;
  double Dx_h3sio4SDx_co2[2] ;
  double Dx_h4sio4SDx_co2[2] ;
  double Dx_cah2sio4SDx_co2[2] ;
  double Dx_cah3sio4SDx_co2[2] ;
  double Dx_nahco3SDx_co2[2] ;
  double Dx_naco3SDx_co2[2] ;
  
  double Dx_hSDx_oh[2] ;
  double Dx_hco3SDx_oh[2] ;
  double Dx_co3SDx_oh[2] ;
  double Dx_caSDx_oh[2] ;
  double Dx_caohSDx_oh[2] ;
  double Dx_cahco3SDx_oh[2] ;
  double Dx_h2sio4SDx_oh[2] ;
  double Dx_h3sio4SDx_oh[2] ;
  double Dx_cah3sio4SDx_oh[2] ;
  double Dx_naohSDx_oh[2] ;
  double Dx_nahco3SDx_oh[2] ;
  ...
  */
  for(i=0;i<2;i++) {
    ...
    {
      /* liquid */
      double dx_co2          =   x_co2*1.e-2 ;
      double *dxsdx_co2 = ComputeAqueousDerivatives(el,x,dx_co2,I_CO2) ;
    
      double dx_hco3sdx_co2  =   dxsdx_co2[I_HCO3] ;
    
      double drho_lsdx_co2   =   dxsdx_co2[I_RHO_L] ;
    
      double dx_c_lsdx_co2   = dxsdx_co2[I_C_L] ;
      double dx_ca_lsdx_co2  = dxsdx_co2[I_Ca_L] ;
      double dx_na_lsdx_co2  = dxsdx_co2[I_Na_L] ;
      double dx_si_lsdx_co2  = dxsdx_co2[I_Si_L] ;
      double dx_qsdx_co2     = dxsdx_co2[I_N_Q] ;

      /* solid */
      double dq_chsdx_co2    =  dxsdx_co2[I_Q_CH] ;
      double tausdt          =  GetProperty("Tau")/dt ;
      double dq_ch_eqsdq_ch  =  ComputeDQ_CH_EQ(el,tausdt,q_ch,Q_CH_EQn(i)) ;
      double dx_cshsdx_co2   =  DX_CSH(q_ch_eq)*dq_ch_eqsdq_ch*dq_chsdx_co2 ;
      double dz_cshsdx_co2   =  DZ_CSH(x_csh)*dx_cshsdx_co2 ;
      double dv_cshsdx_co2   =  DV_CSH(x_csh)*dx_cshsdx_co2 ;
      double dM_CSHsdx_co2   =  dx_cshsdx_co2*M_CaO + dz_cshsdx_co2*M_H2O ;
    
      double dn_chsdx_co2    = - dn1*dx_hco3sdx_co2/x_hco3 ;
      double dn_ca_ssdx_co2  =   dn_chsdx_co2 + dx_cshsdx_co2*n_si_s ;
    
      double dphisdx_co2     = - V_CH*dn_chsdx_co2 - dv_cshsdx_co2*n_si_s ;
    
    
      K(E_C+j,I_CO2+j)    += volume[i]*(phi_g + phi_l*dx_c_lsdx_co2 \
                           + dphisdx_co2*(s_g*x_co2 + s_l*x_c_l)) ;
    
      K(E_mass+j,I_CO2+j) += volume[i]*(phi_g*M_CO2 + phi_l*drho_lsdx_co2 \
                           + dphisdx_co2*(s_l*rho_l + s_g*rho_g) \
                           + M_CaOH2*dn_chsdx_co2 + dM_CSHsdx_co2*n_si_s) ;
    
      K(E_Ca+j,I_CO2+j)   += volume[i]*(phi_l*dx_ca_lsdx_co2 \
                          + dn_ca_ssdx_co2 + dphisdx_co2*s_l*x_ca_l) ;
    
      K(E_el+j,I_CO2+j)   += volume[i]*(dx_qsdx_co2) ;
    
      K(E_Na+j,I_CO2+j)   += volume[i]*(phi_l*dx_na_lsdx_co2 + dphisdx_co2*s_l*x_na_l) ;
    
      K(E_K+j,I_CO2+j)    += volume[i]*(dphisdx_co2*s_l*x_k_l) ;
    
      K(E_Si+j,I_CO2+j)   += volume[i]*(phi_l*dx_si_lsdx_co2 + dphisdx_co2*s_l*x_si_l) ;
    
    
      /* sauvegarde pour termes de transport */
      Dx_h2co3SDx_co2[i]    = dxsdx_co2[I_H2CO3] ;
      Dx_hco3SDx_co2[i]     = dxsdx_co2[I_HCO3] ;
      Dx_co3SDx_co2[i]      = dxsdx_co2[I_CO3] ;
      Dx_caSDx_co2[i]       = dxsdx_co2[I_Ca] ;
      Dx_caohSDx_co2[i]     = dxsdx_co2[I_CaOH] ;
      Dx_caoh2aqSDx_co2[i]  = dxsdx_co2[I_CaOH2aq] ;
      Dx_h2sio4SDx_co2[i]   = dxsdx_co2[I_H2SiO4] ;
      Dx_h3sio4SDx_co2[i]   = dxsdx_co2[I_H3SiO4] ;
      Dx_h4sio4SDx_co2[i]   = dxsdx_co2[I_H4SiO4] ;
      Dx_cah2sio4SDx_co2[i] = dxsdx_co2[I_CaH2SiO4] ;
      Dx_cah3sio4SDx_co2[i] = dxsdx_co2[I_CaH3SiO4] ;
      Dx_nahco3SDx_co2[i]   = dxsdx_co2[I_NaHCO3] ;
      Dx_naco3SDx_co2[i]    = dxsdx_co2[I_NaCO3] ;
    }

    /* ... par rapport a x_oh */
    {
      /* liquid */
      double dx_oh          =   x_oh*1.e-2 ;
      double *dxsdx_oh = ComputeAqueousDerivatives(el,x,dx_oh,I_OH) ;
    
      double drho_lsdx_oh   =   dxsdx_oh[I_RHO_L] ;
         
      double dx_c_lsdx_oh   = dxsdx_oh[I_C_L] ;
      double dx_ca_lsdx_oh  = dxsdx_oh[I_Ca_L] ;
      double dx_na_lsdx_oh  = dxsdx_oh[I_Na_L] ;
      double dx_k_lsdx_oh   = dxsdx_oh[I_K_L] ;
      double dx_si_lsdx_oh  = dxsdx_oh[I_Si_L] ;
      double dx_qsdx_oh     = dxsdx_oh[I_N_Q] ;
    
      K(E_C+j,I_OH+j)    += volume[i]*(phi_l*dx_c_lsdx_oh) ;
    
      K(E_mass+j,I_OH+j) += volume[i]*(phi_l*drho_lsdx_oh) ;
    
      K(E_Ca+j,I_OH+j)   += volume[i]*(phi_l*dx_ca_lsdx_oh) ;
    
      K(E_el+j,I_OH+j)   += volume[i]*(dx_qsdx_oh) ;
    
      K(E_Na+j,I_OH+j)   += volume[i]*(phi_l*dx_na_lsdx_oh) ;
    
      K(E_K+j,I_OH+j)    += volume[i]*(phi_l*dx_k_lsdx_oh) ;
    
      K(E_Si+j,I_OH+j)   += volume[i]*(phi_l*dx_si_lsdx_oh) ;
    
      /* sauvegarde pour termes de transport */
      Dx_hSDx_oh[i]         = dxsdx_oh[I_H] ;
      Dx_hco3SDx_oh[i]      = dxsdx_oh[I_HCO3] ;
      Dx_co3SDx_oh[i]       = dxsdx_oh[I_CO3] ;
      Dx_caSDx_oh[i]        = dxsdx_oh[I_Ca] ;
      Dx_caohSDx_oh[i]      = dxsdx_oh[I_CaOH] ;
      Dx_cahco3SDx_oh[i]    = dxsdx_oh[I_CaHCO3] ;
      Dx_h2sio4SDx_oh[i]    = dxsdx_oh[I_H2SiO4] ;
      Dx_h3sio4SDx_oh[i]    = dxsdx_oh[I_H3SiO4] ;
      Dx_cah3sio4SDx_oh[i]  = dxsdx_oh[I_CaH3SiO4] ;
      Dx_naohSDx_oh[i]      = dxsdx_oh[I_NaOH] ;
      Dx_nahco3SDx_oh[i]    = dxsdx_oh[I_NaHCO3] ;
      Dx_naco3SDx_oh[i]     = dxsdx_oh[I_NaCO3] ;
      Dx_kohSDx_oh[i]       = dxsdx_oh[I_KOH] ;
    }
    
    /* ... par rapport a x_na */
    {
      double dx_na          =  1.e-4 ;
      double *dxsdx_na = ComputeAqueousDerivatives(el,x,dx_na,I_Na) ;
    
      double drho_lsdx_na   = dxsdx_na[I_RHO_L] ;
         
      double dx_c_lsdx_na   = dxsdx_na[I_C_L] ;
      double dx_na_lsdx_na  = dxsdx_na[I_Na_L] ;
      double dx_qsdx_na     = dxsdx_na[I_N_Q] ;
      ...
      /* sauvegarde pour termes de transport */
      Dx_naohSDx_na[i]      = dxsdx_na[I_NaOH] ;
      Dx_nahco3SDx_na[i]    = dxsdx_na[I_NaHCO3] ;
      Dx_naco3SDx_na[i]     = dxsdx_na[I_NaCO3] ;
    }
    
    /* ... par rapport a x_k */
    {
      double dx_k          =  1.e-4 ;
      double *dxsdx_k  = ComputeAqueousDerivatives(el,x,dx_k,I_K) ;
    
      double drho_lsdx_k   =  dxsdx_k[I_RHO_L] ;

      double dx_k_lsdx_k   = dxsdx_k[I_K_L] ;
      double dx_qsdx_k     = dxsdx_k[I_N_Q] ;
      ...
      /* sauvegarde pour termes de transport */
      Dx_kohSDx_k[i]        = dxsdx_k[I_KOH] ;
    }
    
    /* ... par rapport a n_cc */
    {
      double dn_ca_ssdn_cc   = 1 ;
      
      double dphisdn_cc      = - V_CC ;
    
      K(E_C+j,I_CC+j)    += volume[i]*(1 + dphisdn_cc*(s_g*x_co2 + s_l*x_c_l)) ;
    
      K(E_mass+j,I_CC+j) += volume[i]*(dphisdn_cc*(s_l*rho_l + s_g*rho_g) + M_CaCO3) ;
  ...
  */
  ...
  for(i=0;i<2;i++){
    c[i] = trf_co2 + trf_h2co3*Dx_h2co3SDx_co2[i] + trf_hco3*Dx_hco3SDx_co2[i] + trf_co3*Dx_co3SDx_co2[i] + trf_nahco3*Dx_nahco3SDx_co2[i] + trf_naco3*Dx_naco3SDx_co2[i] ;
  }
  K(E_C,I_CO2)          += + c[0] ;
  K(E_C,I_CO2+NEQ)      += - c[1] ;
  K(E_C+NEQ,I_CO2)      += - c[0] ;
  ...
  for(i=0;i<2;i++){
    c[i] = trf_hco3*Dx_hco3SDx_oh[i] + trf_co3*Dx_co3SDx_oh[i] + trf_nahco3*Dx_nahco3SDx_oh[i] + trf_naco3*Dx_naco3SDx_oh[i] + trf_cahco3*Dx_cahco3SDx_oh[i] ;
  }
  K(E_C,I_OH)           += + c[0] ;
  K(E_C,I_OH+NEQ)       += - c[1] ;
  K(E_C+NEQ,I_OH)       += - c[0] ;
  ...
  */
  for(i=0;i<2;i++){
    c[i] = z_hco3*trf_hco3*Dx_hco3SDx_co2[i] + z_co3*trf_co3*Dx_co3SDx_co2[i] + z_ca*trf_ca*Dx_caSDx_co2[i] + z_caoh*trf_caoh*Dx_caohSDx_co2[i] + z_h2sio4*trf_h2sio4*Dx_h2sio4SDx_co2[i] + z_h3sio4*trf_h3sio4*Dx_h3sio4SDx_co2[i] + z_cah3sio4*trf_cah3sio4*Dx_cah3sio4SDx_co2[i] + z_naco3*trf_naco3*Dx_naco3SDx_co2[i] ;
  }
  K(E_q,I_CO2)          += + c[0] ;
  K(E_q,I_CO2+NEQ)      += - c[1] ;
  K(E_q+NEQ,I_CO2)      += - c[0] ;
  ...
  for(i=0;i<2;i++){
    c[i] = z_h*trf_h*Dx_hSDx_oh[i] + z_oh*trf_oh + z_hco3*trf_hco3*Dx_hco3SDx_oh[i] + z_co3*trf_co3*Dx_co3SDx_oh[i] + z_ca*trf_ca*Dx_caSDx_oh[i] + z_caoh*trf_caoh*Dx_caohSDx_oh[i] + z_cahco3*trf_cahco3*Dx_cahco3SDx_oh[i] + z_h2sio4*trf_h2sio4*Dx_h2sio4SDx_oh[i] + z_h3sio4*trf_h3sio4*Dx_h3sio4SDx_oh[i] + z_cah3sio4*trf_cah3sio4*Dx_cah3sio4SDx_oh[i] + z_naco3*trf_naco3*Dx_naco3SDx_oh[i] ;
  }
  K(E_q,I_OH)           += + c[0] ;
  K(E_q,I_OH+NEQ)       += - c[1] ;
  K(E_q+NEQ,I_OH)       += - c[0] ;
  ...
  */
  ...
  for(i=0;i<2;i++){
    c[i] = trf_ca*Dx_caSDx_co2[i] + trf_caoh*Dx_caohSDx_co2[i]+ trf_caoh2aq*Dx_caoh2aqSDx_co2[i] + trf_cah2sio4*Dx_cah2sio4SDx_co2[i] + trf_cah3sio4*Dx_cah3sio4SDx_co2[i] ;
  }
  K(E_Ca,I_CO2)           += + c[0] ;
  ...
  for(i=0;i<2;i++){
    c[i] = trf_ca*Dx_caSDx_oh[i] + trf_caoh*Dx_caohSDx_oh[i] + trf_cahco3*Dx_cahco3SDx_oh[i] + trf_cah3sio4*Dx_cah3sio4SDx_oh[i] ;
  }
  K(E_Ca,I_OH)           += + c[0] ;
  K(E_Ca,I_OH+NEQ)       += - c[1] ;
  K(E_Ca+NEQ,I_OH)       += - c[0] ;
  ...
  */
  ...
  for(i=0;i<2;i++){
    c[i] = trf_nahco3*Dx_nahco3SDx_co2[i] + trf_naco3*Dx_naco3SDx_co2[i] ;
  }
  K(E_Na,I_CO2)          += + c[0] ;
  K(E_Na,I_CO2+NEQ)      += - c[1] ;
  K(E_Na+NEQ,I_CO2)      += - c[0] ;
  K(E_Na+NEQ,I_CO2+NEQ)  += + c[1] ;

  for(i=0;i<2;i++){
    c[i] = trf_naoh*Dx_naohSDx_oh[i] + trf_nahco3*Dx_nahco3SDx_oh[i] + trf_naco3*Dx_naco3SDx_oh[i] ;
  }
  K(E_Na,I_OH)           += + c[0] ;
  K(E_Na,I_OH+NEQ)       += - c[1] ;
  K(E_Na+NEQ,I_OH)       += - c[0] ;
  ...
  */
  ...
  for(i=0;i<2;i++){
    c[i] = trf_h2sio4*Dx_h2sio4SDx_co2[i] + trf_h3sio4*Dx_h3sio4SDx_co2[i] + trf_h4sio4*Dx_h4sio4SDx_co2[i] + trf_cah2sio4*Dx_cah2sio4SDx_co2[i] + trf_cah3sio4*Dx_cah3sio4SDx_co2[i] ;
  }
  K(E_Si,I_CO2)          += + c[0] ;
  ...
  for(i=0;i<2;i++){
    c[i] = trf_h2sio4*Dx_h2sio4SDx_oh[i] + trf_h3sio4*Dx_h3sio4SDx_oh[i] + trf_cah3sio4*Dx_cah3sio4SDx_oh[i] ;
  }
  K(E_Si,I_OH)           += + c[0] ;
  K(E_Si,I_OH+NEQ)       += - c[1] ;
  K(E_Si+NEQ,I_OH)       += - c[0] ;
...
/* Les valeurs exploitees (s) */
{
  ...
  double zero = 0. ;
  double *u[MAX_NOEUDS] ;
#define UNKNOWN_s(i)   UNKNOWN(in,i)
  ...
  */
...

  {
    ...
    double x_csh      = CSR(in) ;
    double *x = ComputeAqueousVariables(el,x_csh,u,in) ;
...
/* Les flux (f) */
{
  ...
  /* Gradients */
  {
    ...
    double grd_na       = (r_na[1]     - r_na[0]    )/dx ;
...
double* ComputeAqueousVariables(Element_t *el,double x_csh,double **u,int n)
{
  double *x = var ;
...
void  ComputeAqueousParameters(Element_t *el,double *x)
{
  ...
  double q_ch       = Q_CH(x_co2) ;
  double x_h4sio4   = IAP_SH(q_ch,x_csh) ;
...
double ComputeCSValue(Element_t *el,double tausdt,double q_ch,double q0)
{
  double q = ComputeQ_CH_EQ(el,tausdt,q_ch,q0) ;
  
  return(X_CSH(q)) ;
}


double ComputeCSDerivative(Element_t *el,double tausdt,double q_ch,double q0)
{
  double dq = ComputeDQ_CH_EQ(el,tausdt,q_ch,q0) ;
  double q = ComputeQ_CH_EQ(el,tausdt,q_ch,q0) ;
...
double ComputeQ_CH_EQ(Element_t *el,double tausdt,double q_ch,double q0)
{
  double err,tol = 1.e-8 ;
  double x0      = X_CSH(q0) ;
  double lnq_ch  = log(q_ch) ;
  double q       = q_ch ;
  int    i = 0 ;
  
  if(q_ch != q0) {
    double x_ch    = X_CSH(q_ch) ;
    double s = tausdt*(x_ch - x0) +  lnq_ch - log(q0) ;
    double p = tausdt*(x_ch - x0) * (lnq_ch - log(q0)) ;
    q *= exp(-p/s) ;
  } else {
    return(q_ch) ;
  }
  ...
  do {
    double x  = X_CSH(q) ;
    double dx = DX_CSH(q)*q ;
    double lnq  = log(q) ;
    double f  = tausdt*(x  - x0) + lnq  - lnq_ch ;
    double df = tausdt*dx + 1 ; 
    double dlnq = - f/df ;
    q  = exp(lnq + dlnq) ;
    if(q > MAX(q_ch,q0)) q = MAX(q_ch,q0) ;
    if(q < MIN(q_ch,q0)) q = MIN(q_ch,q0) ;
    err = fabs(dlnq) ;
    if(i++ > 20) {
      printf("\n") ;
      printf("x0   = %e\n",x0) ;
      ...
      printf("df   = %e\n",df) ;
      printf("dlnq = %e\n",dlnq) ;
      arret("ComputeQ_CH_EQ : no convergence") ;
    }
  } while(err > tol) ;
...
double ComputeDQ_CH_EQ(Element_t *el,double tausdt,double q_ch,double q0)
{
  double q = ComputeQ_CH_EQ(el,tausdt,q_ch,q0) ;
  
  return(q/(q_ch*(tausdt*DX_CSH(q)*q + 1))) ;
}


double ComputeQ_CH_EQ1(Element_t *el,double tausdt,double q_ch,double q0)
{
  ...
  do {
    ...
    double lnq1 = log(q1) ;
    double lnq2 = log(q2) ;
    double f1   = tausdt*(x1 - x0) + lnq1 - lnq_ch ;
    double f2   = tausdt*(x2 - x0) + lnq2 - lnq_ch ;
    double lnq  = (f1 == f2) ? lnq1 : (lnq2*f1 - lnq1*f2)/(f1 - f2) ;
    double q    = exp(lnq) ;
    double x    = X_CSH(q) ;
    double f    = tausdt*(x  - x0) + lnq  - lnq_ch ;
    ...
    if(i++ > 20) {
      ...
      printf("err  = %e\n",err) ;
      printf("f    = %e\n",f) ;
      arret("ComputeQ_CH_EQ1 (2) : no convergence") ;
    }
  } while(err > tol) ;
