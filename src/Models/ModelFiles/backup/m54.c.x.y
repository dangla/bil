...
#define TITLE "Carbonation of cement : CO2-saturated water and alcalis (may 2011)"
...
#define N_CSH0(n)     (va[(32+n)])
#define Z_CH0(n)      (va[(34+n)])
...
#define Y_CSH(q)    courbe(q,el.mat->cb[1])
#define DX_CSH(q)   dcourbe(q,el.mat->cb[0])
#define DY_CSH(q)   dcourbe(q,el.mat->cb[1])
/* Molar Volume */
...
#define IAP_CC(z_co2,z_chcc)         (K_CC*NEGEXP(z_chcc)*MIN(z_co2,1.))
#define IAP_CH(z_co2,z_chcc)         (K_CH*NEGEXP(z_chcc)/MAX(z_co2,1.))
#define IAP_SAM(q_ch,z_csh)          (K_Sam*NEGEXP(z_csh)*F_SAM(q_ch))
#define DIAP_SAMSDQ_CH(q_ch,z_csh)   (K_Sam*NEGEXP(z_csh)*DF_SAM(q_ch))
#define DIAP_SAMSDZ_CSH(q_ch,z_csh)  (K_Sam*DNEGEXP(z_csh)*F_SAM(q_ch))
...
void tb54(elem_t *el,inte_t *fi,int *n_fi,int dim)
{
  el->n_vi = NVI ;
...
/* Initialise les variables du systeme (f,va) */ 
{
  ...
  */
  ...
  for(i=0;i<el.nn;i++) {
    ...
    double P_CC       = IAP_CC(z_co2,z_chcc) ;
    double P_CH       = IAP_CH(z_co2,z_chcc) ;
    double z_ch       = P_CH/K_CH ;
    double P_Sam      = IAP_SAM(z_ch,z_csh) ;
    
    double c_h4sio4   = P_Sam ;
    double c_oh       = concentration_oh(c_co2,el,z_chcc,c_k,z_csh) ;
    ...
    /* contenus en CH, CC, CSH */
    double n_ch       = (z_co2 <= 1) ? n_ch_eq  : 0 ;
    double n_cc       = (z_co2 >  1) ? n_cc_eq  : 0 ;
    double n_csh      = (z_csh > 0.) ? z_csh : 0. ;
    double x_csh      = X_CSH(z_ch) ;
    double y_csh      = Y_CSH(z_ch) ;

    /* solides */
    double n_si_s     = y_csh*n_csh ;
...
/* Les variables donnees par la loi de comportement (f_1) */
{
  ...
  */
  ...
  for(i=0;i<el.nn;i++) {
    ...
    double z_ch0      = Z_CH0(i) ;
    double v_csh0     = V_CSH(z_ch0) ;
    double v_csh      = V_CSH(z_ch) ;
    double phi        = phi0 + V_CH*(n_ch0 - n_ch) + V_CC*(n_cc0 - n_cc) \
                      + v_csh0*n_csh0 - v_csh*n_csh ;
...
#define K(i,j)    (k[(i)*2*NEQ+(j)])
  ...
  */
  for(i=0;i<2;i++) {
    ...
    double c_oh2              = concentration_oh(c_co22,el,z_chcc,c_k,z_csh) ;   
    ...
    double dP_CHsdc_co2       = (z_co2 < 1.) ? 0. : -P_CH/c_co2 ;
    double dP_Samsdc_co2      = DIAP_SAMSDQ_CH(z_ch,z_csh)*dP_CHsdc_co2/K_CH  ;
    
    double dc_casdc_co2       = (dP_CCsdc_co2 - c_ca*dc_co3sdc_co2)/c_co3 ;
    double dc_cahco3sdc_co2   = K_cahco3*(dc_casdc_co2*c_hco3 \
                              + c_ca*dc_hco3sdc_co2) ;

    double dx_cshsdc_co2      = DX_CSH(z_ch)*dP_CHsdc_co2/K_CH ;
    double dy_cshsdc_co2      = DY_CSH(z_ch)*dP_CHsdc_co2/K_CH ;
    ...
    double dn_ca_ssdc_co2     = dn_chsdc_co2 + dn_ccsdc_co2 \
                              + dx_cshsdc_co2*n_csh ;
    double dn_si_ssdc_co2     = dy_cshsdc_co2*n_csh ;

    double dv_cshsdc_co2      = DV_CSH(z_ch)*dP_CHsdc_co2/K_CH ;
    ...
    double c_oh1              = concentration_oh(c_co2,el,z_chcc,c_k,z_csh2) ;
    ...
    double dP_Samsdz_csh      = DIAP_SAMSDZ_CSH(z_ch,z_csh) ;
    ...
    double dn_cshsdz_csh      = (z_csh > 0) ? 1. : 0. ;
    double dn_ca_ssdz_csh     = x_csh*dn_cshsdz_csh ;
    double dn_si_ssdz_csh     = y_csh*dn_cshsdz_csh ;
    
    double dphisdz_csh        = -v_csh*dn_cshsdz_csh ;
    ...
    double c_oh3               = concentration_oh(c_co2,el,z_chcc2,c_k,z_csh) ;
    ...
    double dP_CHsdz_chcc       = (z_chcc < 0) ? P_CH : 0 ;
    double dP_Samsdz_chcc      = DIAP_SAMSDQ_CH(z_ch,z_csh)*dP_CHsdz_chcc/K_CH ;
    ...
    double dx_cshsdz_chcc      = DX_CSH(z_ch)*dP_CHsdz_chcc/K_CH ;
    double dy_cshsdz_chcc      = DY_CSH(z_ch)*dP_CHsdz_chcc/K_CH ;
    double dv_cshsdz_chcc      = DV_CSH(z_ch)*dP_CHsdz_chcc/K_CH ;
    ...
    double dn_si_ssdz_chcc     = dy_cshsdz_chcc*n_csh ;
    double dn_ca_ssdz_chcc     = dn_chsdz_chcc + dn_ccsdz_chcc \
                               + dx_cshsdz_chcc*n_csh ;

    double dphisdz_chcc        = - V_CH*dn_chsdz_chcc \
                               - V_CC*dn_ccsdz_chcc \
                               - dv_cshsdz_chcc*n_csh ;
    ...
    double c_oh4               = concentration_oh(c_co2,el,z_chcc,c_k2,z_csh) ;
...
double concentration_oh(double c_co2,elem_t el,double z_chcc,double c_k,double z_csh)
