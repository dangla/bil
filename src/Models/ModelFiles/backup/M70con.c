...
#define TITLE "Sulfuric acid attack of concrete (May. 2012) + K+Cl" 
#define AUTHORS "Yuan"
...
#define N_CSH2(n)  (f[(21+n)])
#define N_Si_S(n)  (f[(23+n)])
#define ZQ_CH(n)   (f[(25+n)])
#define PHI(n)     (f[(27+n)])
#define PHI_C(n)   (f[(29+n)])
#define P_CSH2(n)  (f[(31+n)])
#define D_con(n)   (f[(33+n)])
...
#define N_CSH2n(n) (f_n[(21+n)])
#define N_Si_Sn(n) (f_n[(23+n)])
#define ZQ_CHn(n)  (f_n[(25+n)])
#define PHIn(n)    (f_n[(27+n)])
#define PHIn_C(n)  (f_n[(29+n)])
#define P_CSH2n(n) (f_n[(31+n)])
#define D_conn(n)  (f_n[(33+n)])
...
#define d_so4      (5.32e-8)    /* 9.53e-8 (radius = 2.25e-10 m) */
#define d_ca       (7.92e-8)
...
#define CC_CSH2    (5.e+06)      /* (??) */
#define CC_Con     (5.e+9)      /* (??) */
#define CS_Con     (3.5e+6)      /* (??) */
/* Porosity of gypsum (-) */
#define phi_CSH2    (0.85)      /*   */
#define V_CSH20     (V_CSH2/(1-phi_CSH2))
...
/* Parametres */
static double phi0,c_h2so4_eq,t_ch,t_csh2,surface ;
...
#define I_C_Ca         (10)
#define I_C_CaOH       (11)
#define I_C_CaHSO4     (12)
#define I_C_CaSO4aq    (13)
...
int pm(char *s)
{
  ...
  /*else if(strcmp(s,"k_int") == 0)    return (6) ;*//* g*/
  else return(-1) ;
}
...
/* Initialise les variables du systeme (f,va) */ 
{
  ...
  */
  ...
  surface       = Element_GetProperty(el)[pm("surface")] ;
  ...
  for(i = 0 ; i < nn ; i++) {
    ...
    /* pressure of gypsum */
    double p_csh20    = CC_Con*CC_CSH2*(n_csh2*V_CSH20-phi_c)/(CC_CSH2+n_csh2*V_CSH20*CC_Con) ;
    double p_csh2     = (((n_csh2*V_CSH20)/phi_c) < 1||xx>surface) ? 0 : p_csh20 ;
...
/* Les variables donnees par la loi de comportement (f_1) */
{
  ...
  */
  ...
  for(i = 0 ; i < nn ; i++) {
    ...
    double damagen    = D_conn(i) ;
    
    double t_chn = (damagen == 1) ? t_ch : (t_ch*10); /*before crack, slow*/
    /*t_ch  = t_chn ;*/
    /*   kinetics */
    double n_ch_ci    = n_chn*pow(zc_h2so4,-dt/t_ch) ;  /* if zc_h2so4 > 1 */
    double n_csh2_ci  = n_csh2n*pow(zc_h2so4,dt/t_csh2) ;   /* if zc_h2so4 < 1 */
    /*   equilibriums */
    double n_ch_eq    = n_ca_ref*MAX(zn_ca_s,0.) - n_csh2_ci ;
    double n_csh2_eq  = n_ca_ref*MAX(zn_ca_s,0.) - n_ch_ci ;
    ...
    /* porosity */
    double n_ch0      = (xx>surface)?n_ca_ref : N_CH0(i) ;
    double n_csh20    =  N_CSH20(i) ;
    double n_si_s0    =  N_Si_S0(i) ;
    double zq_ch0     = (xx>surface)?1 : ZQ_CH0(i) ;
    ...
    double phi_c0       = (xx>surface)? phi0 + V_CH*n_ch0 + v_csh0*n_si_s0 - v_csh*n_si_s: phi0 + V_CH*(n_ch0 - n_ch) + v_csh0*n_si_s0 - v_csh*n_si_s ;
                       
    /*double phia       = phi_c0 - (n_csh2*V_CSH20)*(1-phi_CSH2) ;
    double phib       = phi_c0 *(1-(1-phi_CSH2)/(1-p_csh2/CC_CSH2)) ;
    
    double phi1       = (p_csh2 == 0) ? phia : phib ; */ 
         
    /* pressure of gypsum */
    double p_csh20    = CC_Con*CC_CSH2*(n_csh2*V_CSH20-phi_c0)/(CC_CSH2*phi_c0+n_csh2*V_CSH20*CC_Con) ;
    int    damage     = 0;
    double p_csh2     = (((n_csh2*V_CSH20)/phi_c0) < 1) ? 0 : p_csh20 ;
    
    if (p_csh20 > CS_Con ||damagen == 1) 
       {
		p_csh2    =0;
		damage    =1;
		}
    ...
    double phi_CSH2p   = 1-(1-phi_CSH2) * (1-p_csh2/CC_CSH2);
    double phi_cp      = phi_c0 * (1+p_csh2/CC_Con)   ;
    double phi_sample_f  =  phi_CSH2p * phi_cp;  
    double phi_sample_uf  = phi_cp + V_CSH2*(n_csh20 - n_csh2);  
    
    /*if (p_csh20 > 0.3*CC_CSH2) {p_csh2    =0;}*/
    /*double p_csh2     = (p_csh21 > 0.8*CC_CSH2||xx==1) ? 0 : p_csh21 ; */         
        double v_remainsolid  = nv_agr + v_csh*n_si_s + V_CH*n_ch ;
    double phi_sample_d   =  phi_CSH2 * (1-(v_remainsolid/(v_remainsolid + n_csh2*V_CSH20))); 
    
    
    double n_csh2cr =  phi_c0 / V_CSH20;
    
#if (U_PHI == IMPLICIT)
    double phi        = (((n_csh2*V_CSH20)/phi_c0) < 1)?phi_sample_uf : phi_sample_f;
    /*double phi        = (((n_csh2*V_CSH20)/phi_c0) < 1)?phi_sample_uf : ((damage == 1)? phi_sample_d: phi_sample_f);*/
    double phi_c      = phi_cp ;
#else
    double phi        = PHIn(i) ;
    double phi_c      = PHIn_C(i) ;
#endif
    
    /*if (xx==1) {phi    = phi_CSH2;}*/
    /* liquid contents */
    double n_s_l      = phi*x[I_C_S_L] ;
    double n_ca_l     = phi*x[I_C_Ca_L] ;
    double n_si_l     = phi*x[I_C_Si_L] ;
...
#define K(i,j)    (k[(i)*2*NEQ+(j)])
  ...
  */
  for(i=0;i<nn;i++) {
    ...
    /* pressure of CSH2 */
    double p_csh2     = P_CSH2(i);
    double p_csh2n     = P_CSH2n(i);
    ...
    {
      double dc_h2so4     = 1.e-8*c_h2so4*((c_h2so4 > C_H2SO4n(i)) ? 1 : -1) ;
      ...
      double dn_chsdc_h2so4      = (zc_h2so4 <= 1) ? -dn_csh2_cisdc_h2so4  : dn_ch_cisdc_h2so4 ;
      double dn_csh2sdc_h2so4    = (zc_h2so4 >  1) ? -dn_ch_cisdc_h2so4    : dn_csh2_cisdc_h2so4 ;
      ...
      double dphi_csdc_h2so4     = - V_CH*dn_chsdc_h2so4 - dv_cshsdc_h2so4*n_si_s ;
#else
      double dphisdc_h2so4       = 0 ;
      double dphi_csdc_h2so4     = 0 ;
#endif
      ...
      double dc_cl_lsdc_h2so4    = dcsdc_h2so4[I_C_Cl_L] ;
      ...
      double dn_cl_lsdc_h2so4    = phi*dc_cl_lsdc_h2so4  + dphisdc_h2so4*c_cl_l ;
      ...
      K(E_Cl+j,I_C_H2SO4+j)  += volume[i]*(dn_cl_lsdc_h2so4) ;
      ...
      Dc_csh2SDc_h2so4[i]     = dn_csh2sdc_h2so4;
    }
    ...
    /* with respect to zn_ca_s */
    {
      ...
      Dc_csh2SDzn_ca_s[i]     = dn_csh2sdzn_ca_s ;
    }
        ...
        /* ... with respect to c_cl */
    {
      ...
      Dc_hso4SDc_cl[i]     = dcsdc_cl[I_C_HSO4] ;
      Dc_so4SDc_cl[i]      = dcsdc_cl[I_C_SO4] ;
      Dc_caSDc_cl[i]       = dcsdc_cl[I_C_Ca] ;      
      Dc_cahso4SDc_cl[i]   = dcsdc_cl[I_C_CaHSO4] ;
      ...
      Dc_caso4aqSDc_cl[i]  = dcsdc_cl[I_C_CaSO4aq] ;
  ...
  */
  for(i=0;i<2;i++){
    c[i] = trf_h2so4 + trf_hso4*Dc_hso4SDc_h2so4[i] + trf_so4*Dc_so4SDc_h2so4[i] + trf_cahso4*Dc_cahso4SDc_h2so4[i] \
           + trf_caso4aq*Dc_caso4aqSDc_h2so4[i] + trd_csh2*Dc_csh2SDc_h2so4[i]  ;
  }
  K(E_S,I_C_H2SO4)          += + c[0] ;
  ...
  for(i=0;i<2;i++){
    c[i] = trf_hso4*Dc_hso4SDzn_ca_s[i] + trf_so4*Dc_so4SDzn_ca_s[i] + trf_cahso4*Dc_cahso4SDzn_ca_s[i] + trf_caso4aq*Dc_caso4aqSDzn_ca_s[i] + trd_csh2*Dc_csh2SDzn_ca_s[i]   ;
  }
  K(E_S,I_ZN_Ca_S)           += + c[0] ;
  ...
  for(i=0;i<2;i++){
    c[i] = trf_hso4*Dc_hso4SDc_cl[i] + trf_so4*Dc_so4SDc_cl[i] + trf_cahso4*Dc_cahso4SDc_cl[i] + trf_caso4aq*Dc_caso4aqSDc_cl[i] ;
  }
  K(E_S,I_C_Cl)          += + c[0] ;
  ...
  */
  for(i=0;i<2;i++){
    ...
    + trf_caoh*Dc_caohSDc_h2so4[i] + trf_caso4aq*Dc_caso4aqSDc_h2so4[i] + trd_csh2*Dc_csh2SDc_h2so4[i]   ;
  }
  ...
  for(i=0;i<2;i++){
    ...
    + trf_caoh*Dc_caohSDzn_ca_s[i] + trf_caso4aq*Dc_caso4aqSDzn_ca_s[i] + trd_csh2*Dc_csh2SDzn_ca_s[i]   ;
  }
  ...
  for(i=0;i<2;i++){
    c[i] = trf_ca*Dc_caSDc_cl[i] + trf_cahso4*Dc_cahso4SDc_cl[i] + trf_cah3sio4*Dc_cah3sio4SDc_cl[i] + trf_cah2sio4*Dc_cah2sio4SDc_cl[i] \
    + trf_caoh*Dc_caohSDc_cl[i] + trf_caso4aq*Dc_caso4aqSDc_cl[i] ;
  }
  K(E_Ca,I_C_Cl)         += + c[0] ;
  ...
  */
  ...
  for(i=0;i<2;i++){
    c[i] = z_cl*trf_cl + z_h*trf_h*Dc_hSDc_cl[i] + z_oh*trf_oh*Dc_ohSDc_cl[i] + z_hso4*trf_hso4*Dc_hso4SDc_cl[i] + z_so4*trf_so4*Dc_so4SDc_cl[i] \
           + z_ca*trf_ca*Dc_caSDc_cl[i] + z_cahso4*trf_cahso4*Dc_cahso4SDc_cl[i] + z_h3sio4*trf_h3sio4*Dc_h3sio4SDc_cl[i] \
...
/* Termes explicites (va)  */
{
  ...
  */
  for(i=0;i<2;i++) {
    ...
    /*double iff    = 0 ;
    /* permeabilite */
    /* double k_g    = (k_int/mu_csh2)*pow(phi_c/phi0,3.)*pow(((1-phi0)/(1-phi_c)),2.) ; */
...
/* Les flux (f) */
{
  ...
  /* Gradients */
  {
    ...
    double grd_csh2     = (r_csh2[1]     - r_csh2[0]    )/dx ;
    double j_h2so4      = KF_H2SO4*grd_h2so4;
    /*   producing H2SO4 */
    /*double c_h2so4_i  =  (n_ch0 > 0.1) ? 0 : productionrate(r_h2so4[1],el,r_oh[1],j_h2so4);*/
    double c_h2so4_i  =  0;
    /* Flux */
    double w_h2so4    = - KF_H2SO4*grd_h2so4 + c_h2so4_i  ;
    ...
    W_SO4  = w_so4 ;
    W_S     = w_h2so4 + w_hso4 + w_so4 + w_cahso4 + w_caso4aq ;/* + w_csh2  ;*/
    W_Ca    = w_ca + w_cahso4 + w_cah3sio4 + w_caso4aq + w_caoh + w_cah2sio4; /*w_csh2  ;*/
...
double concentration_oh(double c_h2so4,elem_t *el,double zn_ca_s,double c_k,double c_cl,double zn_si_s)
...
double productionrate(double c_h2so4,elem_t *el, double c_oh,double j_h2so4)
{
	double ph         = 14 + log(c_oh)/log(10.) ;
	...
	/*if (ph>4.5) {return c_h2so4_i = (0.003/86400)*log(10)*c_h;}
	if ((ph>1.5)&&(ph<=4.5)) { return c_h2so4_i = (0.014/86400)*log(10)*c_h;}
	if ((ph>0.5)&&(ph<=1.5)) { return c_h2so4_i = (0.0006/86400)*log(10)*(0.0316/c_h);}
	else return c_h2so4_i = (0.007/86400)*log(10)/100;
    /*if (ph<0) {return c_h2so4_i = 5e-7;}*/
    return c_h2so4_i = (0.0006/86400)*log(10)*100;
...
double* ComputeAqueousVariables(Element_t *el,double **u,int n)
{
  ...
  /*x[I_C_H2SO4]  = (C_H2SO4(n)>1.e-30)? C_H2SO4(n):1.e-30 ;*/
...
void  ComputeAqueousParameters(Element_t *el,double *x)
{
  ...
  double c_oh       = concentration_oh(c_h2so4,el,zn_ca_s,c_k,c_cl,zn_si_s) ;
  double c_h        = K_h2o/c_oh ;
...
double* ComputeAqueousDerivatives1(Element_t *el,double *x,double dxi,int i)
{
  ...
  if(i == I_C_H2SO4) {
    ...
    double cc_oh       = concentration_oh(cc_h2so4,el,zn_ca_s,c_k,c_cl,zn_si_s) ;
    ...
    double cc_oh       = concentration_oh(c_h2so4,el,zzn_ca_s,c_k,c_cl,zn_si_s) ;
    ...
    double cc_oh       = concentration_oh(c_h2so4,el,zn_ca_s,c_k,c_cl,zzn_si_s) ;
    ...
    double dc          = dxi ;
    double cc_k        = c_k + dc ;
    double cc_oh       = concentration_oh(c_h2so4,el,zn_ca_s,cc_k,c_cl,zn_si_s) ;
    ...
    double cc_oh       = concentration_oh(c_h2so4,el,zn_ca_s,c_k,cc_cl,zn_si_s) ;
