...
#if (U_CO2 == LOG_RHO)
  #define C_CO2(n)   (exp(Ln10*UNKNOWN(n,I_CO2)))
  #define C_CO2n(n)  (exp(Ln10*UNKNOWN_n(n,I_CO2)))
#else
  #define C_CO2(n)   (UNKNOWN(n,I_CO2))
  #define C_CO2n(n)  (UNKNOWN_n(n,I_CO2))
#endif
...
/* Initialise les variables du systeme (f,va) */ 
{
  ...
  */
  ...
  c_co2_eq  = Element_GetProperty(el)[pm("C_CO2_eq")] ;
...
#if (U_CO2 == LOG_RHO)
    UNKNOWN(i,I_CO2) = log(c_co2)/Ln10 ;
#else
    C_CO2(i)   = c_co2 ;
#endif
    ZN_Si_S(i) = zn_si_s ;
...
#define K(i,j)    (k[(i)*2*NEQ+(j)])
  ...
  */
  for(i=0;i<nn;i++) {
    ...
    double dP_Samsdc_co2      = DIAP_SHSDQ_CH(zp_ch,zn_si_s)*dP_CHsdc_co2/K_CH  ;
...
#define UNKNOWN_s(i)   UNKNOWN(j,i)
  ...
  */
  ...
  /* output quantities */
  {
    ...
    /* molarities */
#if (U_CO2 == LOG_RHO)
    double c_co2      = exp(Ln10*UNKNOWN_s(I_CO2)) ;
#else
    double c_co2      =  UNKNOWN_s(I_CO2) ;
#endif
    double zc_co2     = c_co2/c_co2_eq ;
    ...
    Result_Store(r + i++,&zc_co2,"zc_co2",1) ;
