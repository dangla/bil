...
#define d_h        (9.311e-7)    /* chemistry handbook (9.311e-7)*/
#define d_co2      (1.91e-7)    /* chemistry handbook (1.91e-7)*/
#define d_h2co3    (7.2e-8)
#define d_hco3     (1.185e-7)    /* chemistry handbook */
#define d_co3      (9.55e-8)    /* 9.53e-8 (radius = 2.25e-10 m) */
#define d_ca       (7.92e-8)    /*7.92e-8*/
#define d_cahco3   (1.07e-7)    /* (radius = 2e-10 m) 1.07e-7*/
#define d_sioh4    (xxx)
#define d_h4sio4   (1.07e-7)    /* 1.07e-7 a modifier*/
#define d_h3sio4   (1.07e-7)    /*(radius = 2e-10 m) a modifier*/
#define d_h2sio4   (1.07e-7)   /* (radius = 2e-10 m) a modifier*/
#define d_cah2sio4 (1.07e-7)    /* a modifier */
#define d_cah3sio4 (1.07e-7)    /* (radius = 2e-10 m) a modifier*/
#define d_caco3aq  (1.43e-7)    /* (radius = 1.5e-10 m) */
#define d_caoh     (1.07e-7)    /* (radius = 2e-10 m) 1.07e-7*/
#define d_k        (1.957e-7)   /* chemistry handbook (1.957e-7)*/
#define d_koh      (1.96e-7)   
#define d_cl       (2.032e-7)   /* chemistry handbook (2.032e-7)*/
#define d_na       (1.334e-7)   /* chemistry handbook */

/* viscosite (Pa.s) */
#define mu_l       	(0.547e-3)      /*unit pa.s t=50°c,www.engineeringtoolbox.com,*/
...
/* Parametres */
static double phi0,c_co2_eq,t_ch,t_cc, k_int, temperature ;
...
int pm(char *s)
{
  ...
  else if(strcmp(s,"temperature") == 0)    return (10) ;
  else return(-1) ;
}


int dm588(int dim,mate_t *mat,FILE *ficd)
...
void tb588(elem_t *el,inte_t *fi,unsigned int *n_fi,int dim)
{
  el->n_vi = NVI ;
...
void ch588(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,double *r,elem_t el,int dim,geom_t geom,double dt,double t,char_t cg)
...
void in588(double **x,double **u,double *f,double *va,elem_t el,int dim,geom_t geom)
...
int ex588(double **x,double **u,double *f,double *va,elem_t el,int dim,geom_t geom,double t) 
...
int ct588(double **x,double **u,double **u_n,double *f,double *f_n,double *va,elem_t el,int dim,geom_t geom,double dt,double t)
...
int mx588(double **x,double **u,double **u_n,double *f,double *f_n,double *va,double *k,elem_t el,int dim,geom_t geom,double dt,double t)
/* Matrice (k) */
{
#define K(i,j)    (k[(i)*2*NEQ+(j)])
  ...
  */
  for(i=0;i<2;i++) {
    ...
    double dn_ch_eqsdp_co2   = 0 ;
    double dn_cc_eqsdp_co2   = 0 ;
    ...
    double dn_ca_ssdp_co2 = dn_chsdp_co2 + dn_ccsdp_co2 + dx_cshsdp_co2 * MAX(zn_si_s,0.) ;
    ...
    double dn_ch_eqsdp_l   = 0 ;
    double dn_cc_eqsdp_l   = 0 ;
    ...
    double dn_ca_ssdp_l = dn_chsdp_l + dn_ccsdp_l + dx_cshsdp_l * MAX(zn_si_s,0.) ;
...
void rs588(double **x,double **u,double **u_n,double *f,double *f_n,double *va,double *r,elem_t el,int dim,geom_t geom,double dt,double t)
/* Residu (r) */
{
...
int so588(double **x,double **u,double *f,double *va,double *s,resu_t *r,elem_t el,int dim,geom_t geom,double t)
/* Les valeurs exploitees (s) */
{
  ...
  */
  ...
  /* quantites exploitees */
  {
    ...
    double gc_h2o     = product_gh2o(p_co2,p_l,temperature) ;
    double gc_co2y    = product_gco2(p_co2,temperature); /*for calculating gc_h2o*/
    double c_co2      = product_co2(p_co2,p_l,temperature);/*(co2 concentration in water)(C_CO2(i) > 0.) ? C_CO2(i) : c_co2_eq ;*/
    double z_co2      = c_co2/c_co2_eq ;
  ...
  /* porosite */
   ...
   double mu_co2   = product_muco2(p_co2,temperature) ; 
    ...
    r[i++].v[0] = y_h2o ;
    strcpy(r[i].text,"gc_co2y") ; r[i].n = 1 ;
    r[i++].v[0] = gc_co2y ;
    strcpy(r[i].text,"gc_h2oy") ; r[i].n = 1 ;
    r[i++].v[0] = y_h2o*gc_co2y ;
  }
...
/* Termes explicites (va)  */
{
  ...
  */
  for(i=0;i<2;i++) {
    ...
    /* permeabilite */
    double k_l  = (k_int/mu_l)*courbe(p_c,el.mat->cb[1])*pow(phi/phi0,3.)*pow(((1.-phi0)/(1.-phi)),2.) ;
    /* permeabilite de gas */
    double mu_co2   = product_muco2(p_co2,temperature) ;
    double k_g  = (k_int/mu_co2)*courbe(p_c,el.mat->cb[2])*pow(phi/phi0,3.)*pow(((1.-phi0)/(1.-phi)),2.) ;
