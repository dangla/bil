...
#define ConcentrationOfOHInLiquid(A,B,C)  concentration_oh(A,B,C,el)
...
/* Parametres */
static double phii,k_int,a_2,c_2,n_ch0,x_na0,x_k0,frac,phi_r ;
static double p_g = 0. ;
...
void ComputePhysicoChemicalProperties(double TK)
{
  ...
  /* Physical constants */
  {
    double RT      = PhysicalConstants(PerfectGasConstant)*TK*1.e3 ;
    double Faraday = PhysicalConstants(Faraday)*1.e3 ;
    FRT     = Faraday/RT ;
  }
}
...
void  ComputeSecondaryComponents(Element_t *el,double dt,double *x)
{
  ...
  /* Liquid components */
  double x_oh       = ConcentrationOfOHInLiquid(x_co2,x_na,x_k) ;
  double x_h        = k_e/x_oh ;
