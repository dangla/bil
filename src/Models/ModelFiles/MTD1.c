...
#define TITLE   "Carbonation Of CBM (2015)"
...
#define k_naoh        (6.60693448e-15)        /* Na[+] + H2O = NaOH[0] + H[+] */
...
static void    ComputeSecondaryComponents1(Element_t*,double,double*) ;
...
void  ComputeSecondaryComponents(Element_t *el,double dt,double *x)
{
  ...
  double n_cc       = (z_co2 > 1) ? n_ch_cc - n_ch_ki : 0 ;
...
/* Solve for x_h the electroneutrality equation: SUM(z_i c_i) = 0
...
double concentration_oh1(double x_co2,double zn_ca_s,double zn_si_s,double x_na,double x_k,Element_t *el)
/* Solve for x_h the electroneutrality equation: SUM(z_i c_i) = 0
   as root of ax^4 + bx^3 + cx^2 + dx + e = 0 */
{
  /* The primary variables are considered as constant */
