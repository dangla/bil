...
#define MODELINDEX  19
#define TITLE "Poroplasticite ecrouissable non sature (GDR Momas)"
...
static int    c19(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,elem_t el,int dim,geom_t geom,double dt,double *c) ;
static int    k19(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,elem_t el,int dim,geom_t geom,double dt,double *c) ;
static double rn19(double *,double *,double *,mate_t) ;
static double cr19(double *,double,double *,double *,double *,mate_t) ;
/* Parametres */
static double gravite,young,poisson,phi0,k_int,mu_l,rho_l0,p_l0,b,N,rho_s,c,af,ad,beta,k_l,sig0_11,sig0_22,sig0_33,alpha,gam_R,a_int,p_g ;
...
int pm(const char *s)
{
  ...
  else if(strcmp(s,"alpha") == 0) return (19) ;
  else if(strcmp(s,"gamma_R") == 0) return (20) ;
  else if(strcmp(s,"a_int") == 0) return (21) ;
  else if(strcmp(s,"n_p") == 0) return (22) ;
  else if(strcmp(s,"p_c1") == 0) return (23) ;
  else if(strcmp(s,"p_c2") == 0) return (24) ;
  else if(strcmp(s,"p_g") == 0) return (25) ;
  else if(strcmp(s,"courbes") == 0) return (26) ;
  else { printf("donnee \"%s\" non connue (pm19)\n",s) ; exit(0) ; }
}

int dm19(int dim,mate_t *mat,FILE *ficd)
...
void tb19(elem_t *el,inte_t *fi,unsigned int *n_fi,int dim)
{
  el->n_vi = NVE*el->fi->np ;
  el->n_ve = NVA*el->fi->np ;
}

void ch19(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,double *r,elem_t el,int dim,geom_t geom,double dt,double t,char_t cg)
...
void in19(double **x,double **u,double *f,double *va,elem_t el,int dim,geom_t geom)
/* Initialise les variables du systeme (f,va) */ 
{
  double dmu,lame,rho_l,k_h ;
  double eps[9],sig[9],phi,dphi,pl,dpl,tre,m_l,w_l[3],gpl[3],eps_p[9],gam_p,pp,pp_0,pc,sl ;
  double dfsds[9],dgsds[9],hm,crit ;
  int    i,p ;
  double *h,*dh ;
  ...
  */
  ...
  for(p=0;p<el.fi->np;p++) {
    ...
    /* porosite */
    dphi = b*tre + sl*N*dpl ; /* normalement dphi = b*tre + N*(pp - pp_0) */
    phi  = phi0 + dphi ;
    /* contraintes effectives */
    for(i=0;i<9;i++) sig[i] = dmu*eps[i] ;
    sig[0] += sig0_11 + lame*tre - b*(pp - pp_0) + beta*pp ;
    sig[4] += sig0_22 + lame*tre - b*(pp - pp_0) + beta*pp ;
    sig[8] += sig0_33 + lame*tre - b*(pp - pp_0) + beta*pp ;
    /* deformations plastiques */
    for(i=0;i<9;i++) eps_p[i] = 0. ;
    gam_p = 0. ;
    /* critere */
    crit = cr19(sig,gam_p,dfsds,dgsds,&hm,*el.mat) ;
    /* contraintes totales */
    sig[0] += - beta*pp ;
    sig[4] += - beta*pp ;
    sig[8] += - beta*pp ; 
    ...
    k_h = rho_l*k_int/mu_l*courbe(pc,el.mat->cb[1]) ;
    if(dphi > 0) {
      if(dphi < 1.e-2) {
...
int ex19(double **x,double **u,double *f,double *va,elem_t el,int dim,geom_t geom,double t) 
/* Termes explicites (va)  */
{
  double rho_l,k_h ;
  double eps[9],eps_p[9],dphi,pl,tre,phi_p,tre_p,pc,sl ; 
  int    i,p ;
  double *h,*dh ;
  ...
  */
  ...
  for(p=0;p<el.fi->np;p++) {
    ...
    phi_p = beta*tre_p ;
    dphi  = b*(tre - tre_p) + sl*N*(pl - p_l0) + phi_p ;
    /* masse volumique du fluide */
...
int ct19(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,elem_t el,int dim,geom_t geom,double dt,double t)
/* Les variables donnees par la loi de comportement (f_1) */
{
  double dmu,lame,mu,rho_l,k_h ;
  double eps[9],sig[9],phi,pl,tre,m_l,w_l[3],gpl[3],eps_p[9],phi_p,tre_p,gam_p,sl,pc,pp,pp_n ;
  double eps_n[9],pl_n ;
  ...
  */
  ...
  for(p=0;p<el.fi->np;p++) {
    ...
    sig[0] += lame*trde - b*(pp - pp_n) + beta*pp ;
    sig[4] += lame*trde - b*(pp - pp_n) + beta*pp ;
    sig[8] += lame*trde - b*(pp - pp_n) + beta*pp ;
    /* deformations plastiques */
    for(i=0;i<9;i++) eps_p[i] = f_n[p*NVE+16+i] ;
    gam_p = f_n[p*NVE+26] ;
    /* projection */
    crit = rn19(sig,eps_p,&gam_p,*el.mat) ;
    /* contraintes totales */
    sig[0] -= beta*pp ;
    sig[4] -= beta*pp ;
    sig[8] -= beta*pp ;
    ...
    tre_p = eps_p[0] + eps_p[4] + eps_p[8] ;
    phi_p = beta*tre_p ;
    phi   = phi0 + b*(tre - tre_p) + sl*N*(pl - p_l0) + phi_p ;
    /* masse volumique du fluide */
    rho_l = rho_l0*(1. + (pl - p_l0)/k_l) ;
    /* masse liquide */
    m_l = rho_l*phi*sl ;
...
int mx19(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,double *k,elem_t el,int dim,geom_t geom,double dt,double t)
/* Matrice (k) */
{
#define K(i,j)    (k[(i)*el.nn*NEQ+(j)])
  ...
  */
  dec = c19(x,u_1,u_n,f_1,f_n,va,el,dim,geom,dt,c) ;
  /* 
  ** 1.  Mecanique
  */
  ...
  */
  dec = k19(x,u_1,u_n,f_1,f_n,va,el,dim,geom,dt,c) ;
...
void rs19(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,double *r,elem_t el,int dim,geom_t geom,double dt,double t)
/* Residu (r) */
{
...
int so19(double **x,double **u,double *f,double *va,double *s,resu_t *r,elem_t el,int dim,geom_t geom,double t)
/* Les valeurs exploitees (r) */
{
  int    i,j,p,nso ;
  ...
  */
  ...
  for(p=0;p<el.fi->np;p++) {
    ...
    strcpy(r[5].text,"gamma_p") ; r[5].n = 1 ;
    r[5].v[0] += f[p*NVE+26]/el.fi->np ;
    strcpy(r[7].text,"k_h") ; r[7].n = 1 ;
    r[7].v[0] += va[p*NVA]/el.fi->np ;
  }
  return (nso) ;
}
...
int c19(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,elem_t el,int dim,geom_t geom,double dt,double *c)
/*
...
#define B1(i,j)      (c1[(i)*3+(j)])
  double dmu,lame,mu ;
  double pl,rho_l,pc,sl,dslsdpc,dppsdpl,pp ;
  double sig[9],crit,eps_p[9],hm,phi,eps[9],tre,tre_p,phi_p,gam_p ;
  double dfsds[3][3],dgsds[3][3],fc[3][3],cg[3][3],fcg,trf,trg ;
  int    dec ;
  int    i,j,k,l,p ;
  double *h,*dh,*c1 ;
  double zero = 0.,un = 1.,deux = 2. ;
  ...
  */
  ...
  for(p=0;p<el.fi->np;p++) {
    ...
    crit = f_1[p*NVE+25] ;
    if(crit >= 0.) {
      cr19(sig,gam_p,dfsds[0],dgsds[0],&hm,*el.mat) ;
      fcg = hm ;
      for(i=0;i<3;i++) for(j=0;j<3;j++) {
	fc[i][j] = 0. ;
	cg[i][j] = 0. ;
    ...
    c1 += 81 ;
    for(i=0;i<3;i++) B1(i,i) = -b*dppsdpl ;
    if(crit >= 0.) {
      trf = dfsds[0][0] + dfsds[1][1] + dfsds[2][2] ;
      for(i=0;i<3;i++) for(j=0;j<3;j++) {
	B1(i,j) -= cg[i][j]*(beta-b)*trf*fcg*dppsdpl ;
      }
    }
    
    c1 += 9 ;
    /* hydraulique */
    for(i=0;i<3;i++) B1(i,i) = sl*rho_l*b ;
    if(crit >= 0.) {
      trg = dgsds[0][0] + dgsds[1][1] + dgsds[2][2] ;
      for(i=0;i<3;i++) for(j=0;j<3;j++) {
	B1(i,j) += sl*rho_l*fc[i][j]*(beta-b)*trg*fcg ;
      }
    }
    c1 += 9 ;
    c1[0] = sl*sl*rho_l*N + sl*rho_l0*phi/k_l - rho_l*(sl*N*(pl - p_l0) + phi)*dslsdpc ;
    if(crit >= 0.) {
      c1[0] += sl*rho_l*(beta-b)*(beta-b)*trf*trg*fcg*dppsdpl ;
    }
  }
  return(dec) ;
...
int k19(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,elem_t el,int dim,geom_t geom,double dt,double *c)
/*
...
double cr19(double *sig,double gam_p,double *dfsds,double *dgsds,double *hm,mate_t mat)
/* Critere de Drucker-Prager */
{
  double id[9] = {1,0,0,0,1,0,0,0,1} ;
  double p,q,dev[9] ;
  double crit,dl ;
  double ff,dd,cc,k,dmu,mu,c1,cc0 ;
  int    i ;
  /*
    Donnees
  */
  ...
  c       = mat.pr[pm("cohesion")] ;
  af      = mat.pr[pm("frottement")]*M_PI/180. ;
  ad      = mat.pr[pm("dilatance")]*M_PI/180. ;
  alpha   = mat.pr[pm("alpha")] ;
  gam_R   = mat.pr[pm("gamma_R")] ;
  dmu     = young/(1.+poisson) ;
  mu      = dmu/2. ;
  ...
  */
  ...
  cc0     = 6.*cos(af)/(3. - sin(af))*c ;
  /*
    La cohesion
  */
  c1 = (gam_p < gam_R) ? 1.-(1.-alpha)*gam_p/gam_R : alpha ;
  cc = cc0*c1*c1 ;
  /*
    Le critere
  */ 
  ...
  */
  ...
  if(crit > 0.) {
    ...
    /* regime de coin */
      ...
      dl = (ff*p - cc)/(k*ff*dd) ;
      for(i=0;i<9;i++) dgsds[i] = dev[i]/(dmu*dl) + id[i]*dd/3. ;
    }
  }
...
double rn19(double *sig,double *eps_p,double *gam_p,mate_t mat)
/* Critere de Drucker-Prager : return mapping */
{
  double id[9] = {1,0,0,0,1,0,0,0,1} ;
  double tol = 1.e-8 ;
  double p_t,q_t,p,q,dev_t[9] ;
  double gam_n,deps_p[9] ;
  double crit,df,dl,fcrit,dqsdl,dpsdl,dccsdl ;
  double ff,dd,cc,k,dmu,mu,c1,cc0 ;
  int    i,nf ;
  /*
    Donnees
  ...
  */
  ...
  if(crit > 0.) {
    q = q_t ;
    /* cas regulier : supposons que q > 0 */
    if(q > 0) {
      fcrit = crit ;
      nf    = 0 ;
      dl    = 0. ;
      while(abs(fcrit) > tol*cc0) {
	/* deps_p */
	for(i=0;i<9;i++) {
	  deps_p[i] = dl*(1.5*dev_t[i]/q_t + id[i]*dd/3.) ;
	}
	/* p, q, cc */
	(*gam_p) = gam_n + sqrt(2*j2(deps_p)) ;
	c1 = ((*gam_p) < gam_R) ? 1.-(1.-alpha)*(*gam_p)/gam_R : alpha ;
	cc = cc0*c1*c1 ;
	q  = q_t - dl*3*mu ;
	p  = p_t - dl*k*dd ;
	...
	if((*gam_p) < gam_R) {
	  ...
	  dccsdl *= 1.5*sqrt(2*j2(dev_t))/q_t ;
	}
	/* dl */
	fcrit = q + ff*p - cc ;
	df    = dqsdl + ff*dpsdl - dccsdl ;
	dl   -= fcrit/df ;
	if(nf++ > 20) {
	  printf("pas de convergence (rn19)") ;
	  exit(0) ;
	}
      }
      /* sig */
    ...
    if(q <= 0.) {
      /* le deviateur deps_p */
      for(i=0;i<9;i++) deps_p[i]  = dev_t[i]/dmu ;
      /* p, q, cc */
      (*gam_p) = gam_n + sqrt(2*j2(deps_p)) ;
      c1 = ((*gam_p) < gam_R) ? 1.-(1.-alpha)*(*gam_p)/gam_R : alpha ;
      cc = cc0*c1*c1 ;
      ...
      /* deps_p, sig */
      for(i=0;i<9;i++) {
	deps_p[i] = dev_t[i]/dmu + dl*id[i]*dd/3. ;
	sig[i]    = p*id[i] ;
      }
    }
