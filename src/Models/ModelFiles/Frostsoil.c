...
#define M_SALT(vi)    (vi + 4)[0]
#define W_SALT(vi)    (vi + 5)

#define S_TOT(vi)     (vi + 8)[0]
#define W_THE(vi)     (vi + 9)

#define SIG(vi)       (vi + 12) /* this a 3D tensor */
#define F_MASS(vi)    (vi + 21)



/* We define some names for explicit terms */
#define NVE   (5)     /*  nb of explicit terms per point */
#define KD_LIQ(ve)    (ve)[0]
#define KF_SALT(ve)   (ve + 1)[0]
#define KTH(ve)       (ve + 2)[0]
#define MC_SALT(ve)   (ve + 3)[0]
#define S_L(ve)       (ve + 4)[0]
  ...
  /* Loop on integration points */
  {
    ...
    for(p = 0 ; p < NbOfIntPoints ; p++) {
      ...
      /* storage in vi */
      {
        ...
        M_TOT(vi)   = x[I_M_TOT] ;
        M_SALT(vi)  = x[I_M_SALT] ;
        S_TOT(vi)   = x[I_S_TOT] ;
    
        for(i = 0 ; i < 3 ; i++) W_TOT(vi)[i]  = x[I_W_TOT + i] ;
        for(i = 0 ; i < 3 ; i++) W_SALT(vi)[i] = x[I_W_SALT + i] ;
        for(i = 0 ; i < 3 ; i++) W_THE(vi)[i]  = x[I_W_THE + i] ;
        for(i = 0 ; i < 9 ; i++) SIG(vi)[i]    = x[I_SIG + i] ;
        for(i = 0 ; i < 3 ; i++) F_MASS(vi)[i] = x[I_Fmass + i] ;
      }
      ...
      /* storage in ve */
      {
        ...
        KD_LIQ(ve)  = x[I_KD_LIQ] ;
        KF_SALT(ve) = x[I_KF_SALT] ;
        KTH(ve)     = x[I_KTH] ;
        MC_SALT(ve) = x[I_MC_SALT] ;
        S_L(ve)     = x[I_S_L] ;
      }
    }
  }
...
#define R(n,i)    (r[(n)*NEQ + (i)])
  ...
  {
    double* vi = vi1 ;
    double* rw = FEM_ComputeStrainWorkResidu(fem,intfct,SIG(vi),NVI) ;
    int i ;
  ...
  {
    double* vi = vi1 ;
    double* rbf = FEM_ComputeBodyForceResidu(fem,intfct,F_MASS(vi) + dim - 1,NVI) ;
    int i ;
  ...
  if(Element_EquationIsActive(el,E_Mass)) 
  {
    ...
    for(i = 0 ; i < np ; i++ , vi += NVI , vi_n += NVI) g1[i] = M_TOT(vi) - M_TOT(vi_n) ;
  ...
  {
    double* vi = vi1 ;
    double* rf = FEM_ComputeFluxResidu(fem,intfct,W_TOT(vi),NVI) ;
    int i ;
  ...
  if(Element_EquationIsActive(el,E_Salt)) 
  {
    ...
    for(i = 0 ; i < np ; i++ , vi += NVI , vi_n += NVI) g1[i] = M_SALT(vi) - M_SALT(vi_n) ;
  ...
  {
    double* vi = vi1 ;
    double* rf = FEM_ComputeFluxResidu(fem,intfct,W_SALT(vi),NVI) ;
    int i ;
  ...
  if(Element_EquationIsActive(el,E_The)) 
  {
    ...
    for(i = 0 ; i < np ; i++ , vi += NVI , vi_n += NVI) g1[i] = S_TOT(vi) - S_TOT(vi_n) ;
  ...
  {
    double* vi = vi1 ;
    double* rf = FEM_ComputeFluxResidu(fem,intfct,W_THE(vi),NVI) ;
    int i ;
  ...
  */
  ...
  
  {
    ...
    for(i = 0 ; i < np ; i++ , vi += NVI , ve += NVE) {
      ...
      for(j = 0 ; j < 3 ; j++) w_tot[j]  += W_TOT(vi)[j]/np ;
      for(j = 0 ; j < 3 ; j++) w_salt[j] += W_SALT(vi)[j]/np ;
      for(j = 0 ; j < 3 ; j++) w_the[j]  += W_THE(vi)[j]/np ;
      for(j = 0 ; j < 9 ; j++) sig[j]    += SIG(vi)[j]/np ;
    }
 ...
 *  Return a pointer on the locally defined array of the variables. */
{
  ...
  /* Needed variables to compute secondary variables */
  {
    ...
    /* Transfer coefficient */
    {
      ...
      x_n[I_KD_LIQ]  = KD_LIQ(ve) ;
      x_n[I_KF_SALT] = KF_SALT(ve) ;
      x_n[I_KTH]     = KTH(ve) ;
      x_n[I_MC_SALT] = MC_SALT(ve) ;
      x_n[I_S_L]     = S_L(ve) ;
    }
...
/** Compute the secondary variables from the primary ones. */
{
  ...
  /* Fluxes */
  {
    ...
    double  kd_liq  = x_n[I_KD_LIQ] ;
    double  kf_salt = x_n[I_KF_SALT] ;
    double  kth     = x_n[I_KTH] ;
    double  mc_salt = x_n[I_MC_SALT] ;
