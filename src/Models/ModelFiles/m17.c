...
#define MODELINDEX  17
#define TITLE "Poroplasticite ecrouissable (GDR Momas)"
...
static int    c17(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,elem_t el,int dim,geom_t geom,double dt,double *c) ;
static int    k17(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,elem_t el,int dim,geom_t geom,double dt,double *c) ;
static double rn17(double *,double *,double *,mate_t) ;
static double cr17(double *,double,double *,double *,double *,mate_t) ;
/* Parametres */
static double gravite,young,poisson,phi0,k_int,mu_l,rho_l0,p_l0,b,N,rho_s,c,af,ad,beta,k_l,sig0_11,sig0_22,sig0_33,alpha,gam_R,a_int ;
...
int pm(const char *s)
{
  ...
  else { printf("donnee \"%s\" non connue (pm17)\n",s) ; exit(0) ; }
}

int dm17(int dim,mate_t *mat,FILE *ficd)
...
void tb17(elem_t *el,inte_t *fi,unsigned int *n_fi,int dim)
{
  el->n_vi = NVE*el->fi->np ;
  el->n_ve = NVA*el->fi->np ;
}

void ch17(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,double *r,elem_t el,int dim,geom_t geom,double dt,double t,char_t cg)
...
void in17(double **x,double **u,double *f,double *va,elem_t el,int dim,geom_t geom)
/* Initialise les variables du systeme (f,va) */ 
{
  double dmu,lame,rho_l,k_h ;
  double eps[9],sig[9],phi,dphi,pl,dpl,tre,m_l,w_l[3],gpl[3],eps_p[9],gam_p ;
  ...
  */
  ...
  for(p=0;p<el.fi->np;p++) {
    ...
    gam_p = 0. ;
    /* critere */
    crit = cr17(sig,gam_p,dfsds,dgsds,&hm,*el.mat) ;
    /* contraintes totales */
    sig[0] += - beta*pl ;
    sig[4] += - beta*pl ;
    sig[8] += - beta*pl ; 
...
int ex17(double **x,double **u,double *f,double *va,elem_t el,int dim,geom_t geom,double t) 
...
int ct17(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,elem_t el,int dim,geom_t geom,double dt,double t)
/* Les variables donnees par la loi de comportement (f_1) */
{
  double dmu,lame,mu,rho_l,k_h ;
  double eps[9],sig[9],phi,pl,tre,m_l,w_l[3],gpl[3],eps_p[9],phi_p,tre_p,gam_p ;
  double eps_n[9],pl_n ;
  ...
  */
  ...
  for(p=0;p<el.fi->np;p++) {
    ...
    gam_p = f_n[p*NVE+26] ;
    /* projection */
    crit = rn17(sig,eps_p,&gam_p,*el.mat) ;
    /* contraintes totales */
    sig[0] -= beta*pl ;
    sig[4] -= beta*pl ;
    sig[8] -= beta*pl ;
...
int mx17(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,double *k,elem_t el,int dim,geom_t geom,double dt,double t)
/* Matrice (k) */
{
#define K(i,j)    (k[(i)*el.nn*NEQ+(j)])
  ...
  */
  dec = c17(x,u_1,u_n,f_1,f_n,va,el,dim,geom,dt,c) ;
  if(dec < 0) return(1) ;
  /* 
  ** 1.  Mecanique
  */
  ...
  */
  dec = k17(x,u_1,u_n,f_1,f_n,va,el,dim,geom,dt,c) ;
  if(dec < 0) return(1) ;
...
void rs17(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,double *r,elem_t el,int dim,geom_t geom,double dt,double t)
/* Residu (r) */
{
...
int so17(double **x,double **u,double *f,double *va,double *s,resu_t *r,elem_t el,int dim,geom_t geom,double t)
/* Les valeurs exploitees (r) */
{
  ...
  for(p=0;p<el.fi->np;p++) {
    ...
    strcpy(r[6].text,"k_h") ; r[6].n = 1 ;
    r[6].v[0] += va[p*NVA]/el.fi->np ;
  }
  return (nso) ;
}

int c17(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,elem_t el,int dim,geom_t geom,double dt,double *c)
/*
...
#define B1(i,j)      (c1[(i)*3+(j)])
  double dmu,lame,mu ;
  double sig[9],pl,rho_l,crit,eps_p[9],hm,phi,eps[9],tre,tre_p,phi_p,gam_p,crit_1 ;
  ...
  */
  ...
  for(p=0;p<el.fi->np;p++) {
    ...
    crit = f_1[p*NVE+25] ;
    if(crit >= 0.) {
      crit_1 = cr17(sig,gam_p,dfsds[0],dgsds[0],&hm,*el.mat) ;
      fcg = hm ;
      for(i=0;i<3;i++) for(j=0;j<3;j++) {
	fc[i][j] = 0. ;
	cg[i][j] = 0. ;
...
int k17(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,elem_t el,int dim,geom_t geom,double dt,double *c)
/*
...
double cr17(double *sig,double gam_p,double *dfsds,double *dgsds,double *hm,mate_t mat)
...
double rn17(double *sig,double *eps_p,double *gam_p,mate_t mat)
/* Critere de Drucker-Prager : return mapping */
{
  double id[9] = {1,0,0,0,1,0,0,0,1} ;
  double tol = 1.e-8 ;
  double p_t,q_t,p,q,sdev_t[9] ;
  ...
  */ 
  ...
  /*
    Le deviateur
  */
  for(i=0;i<9;i++) sdev_t[i] = sig[i] - p_t*id[i] ;
  /*
  ...
  */
  ...
  if(crit > 0.) {
	...
	dl   -= fcrit/df ;
	if(nf++ > 20) {
	  printf("pas de convergence (rn17)") ;
	  exit(0) ;
	}
      }
      /* sig */
