...
#define MODELINDEX  18
#define TITLE "Poroplasticite non sature (Drucker-Prager)"
...
static int    c18(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,elem_t el,int dim,geom_t geom,double dt,double *c) ;
static int    k18(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,elem_t el,int dim,geom_t geom,double dt,double *c) ;
static double rn18(double *,double *,mate_t) ;
static double cr18(double *,double *,double *,mate_t) ;
/* Parametres */
static double gravite,young,poisson,phi0,k_int,mu_l,rho_l0,p_l0,b,N,rho_s,c,af,ad,beta,k_l,sig0_11,sig0_22,sig0_33,a_int,p_g ;
...
int pm(const char *s)
{
  ...
  else if(strcmp(s,"n_p") == 0) return (20) ;
  else if(strcmp(s,"p_c1") == 0) return (21) ;
  else if(strcmp(s,"p_c2") == 0) return (22) ;
  else if(strcmp(s,"p_g") == 0) return (23) ;
  else if(strcmp(s,"courbes") == 0) return (24) ;
  else { printf("donnee \"%s\" non connue (pm18)\n",s) ; exit(0) ; }
}

int dm18(int dim,mate_t *mat,FILE *ficd)
...
void tb18(elem_t *el,inte_t *fi,unsigned int *n_fi,int dim)
{
  el->n_vi = NVE*el->fi->np ;
  el->n_ve = NVA*el->fi->np ;
}

void ch18(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,double *r,elem_t el,int dim,geom_t geom,double dt,double t,char_t cg)
...
void in18(double **x,double **u,double *f,double *va,elem_t el,int dim,geom_t geom)
/* Initialise les variables du systeme (f,va) */ 
{
  double dmu,lame,rho_l,k_h ;
  double eps[9],sig[9],phi,dphi,pl,dpl,tre,m_l,w_l[3],gpl[3],eps_p[9],pp,pp_0,pc,sl ; 
  ...
  */
  ...
  for(p=0;p<el.fi->np;p++) {
    ...
    /* critere */
    crit = cr18(sig,dfsds,dgsds,*el.mat) ;
    /* contraintes totales */
    sig[0] += - beta*pp ;
    sig[4] += - beta*pp ;
    sig[8] += - beta*pp ; 
...
int ex18(double **x,double **u,double *f,double *va,elem_t el,int dim,geom_t geom,double t) 
...
int ct18(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,elem_t el,int dim,geom_t geom,double dt,double t)
/* Les variables donnees par la loi de comportement (f_1) */
{
  double dmu,lame,mu,rho_l,k_h ;
  double eps[9],sig[9],phi,pl,tre,m_l,w_l[3],gpl[3],eps_p[9],phi_p,tre_p,pp,pp_n,sl,pc ;
  double eps_n[9],pl_n ;
...
int mx18(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,double *k,elem_t el,int dim,geom_t geom,double dt,double t)
/* Matrice (k) */
{
#define K(i,j)    (k[(i)*el.nn*NEQ+(j)])
  ...
  */
  dec = c18(x,u_1,u_n,f_1,f_n,va,el,dim,geom,dt,c) ;
  /* 
  ** 1.  Mecanique
  */
  ...
  */
  dec = k18(x,u_1,u_n,f_1,f_n,va,el,dim,geom,dt,c) ;
...
void rs18(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,double *r,elem_t el,int dim,geom_t geom,double dt,double t)
/* Residu (r) */
{
...
int so18(double **x,double **u,double *f,double *va,double *s,resu_t *r,elem_t el,int dim,geom_t geom,double t)
/* Les valeurs exploitees (s) */
{
  ...
  */
  ...
  strcpy(r[5].text,"saturation") ; r[5].n = 1 ;
  r[5].v[0] = sl ;
  strcpy(r[6].text,"pie") ; r[6].n = 1 ;
  r[6].v[0] = pp ;
...
int c18(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,elem_t el,int dim,geom_t geom,double dt,double *c)
/*
...
#define B1(i,j)      (c1[(i)*3+(j)])
  ...
  */
  ...
  for(p=0;p<el.fi->np;p++) {
    ...
    crit = f_1[p*NVE+25] ;
    if(crit >= 0.) {
      cr18(sig,dfsds[0],dgsds[0],*el.mat) ;
      fcg = 0. ;
      for(i=0;i<3;i++) for(j=0;j<3;j++) {
	fc[i][j] = 0. ;
	cg[i][j] = 0. ;
...
int k18(double **x,double **u_1,double **u_n,double *f_1,double *f_n,double *va,elem_t el,int dim,geom_t geom,double dt,double *c)
/*
...
double cr18(double *sig,double *dfsds,double *dgsds,mate_t mat)
...
double rn18(double *sig,double *eps_p,mate_t mat)
