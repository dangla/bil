...
int pm(const char *s)
{
  ...
  else if(strcmp(s,"K_s") == 0)        return (24) ;
...
/**  Tangent matrix coefficients (c) */
{
  ...
  for(i = 0 ; i < nn ; i++) {
    ...
    #if defined (U_C_SO4)
      #if (U_SO4 == NOLOG_U)
        dui[U_C_SO4] =  1.e-2*ObVal_GetValue(obval + U_C_SO4)/(Ln10*xi[U_C_SO4]) ;
      #endif
    #endif
...
void  ComputeSecondaryVariables(Element_t* el,double t,double dt,double* x)
{
  ...
  /* Kinetic law */
  double varphi_c  = varphi_cn + dt*PoreCrystalGrowthRate(s_c,s_aft,beta_p) ;
  /* Compute the strain */
...
double Radius(double r_n,double s_aft,double dt,Element_t* el)
{
  ...
  
  {
    ...
    for(i = 0 ; i < iterations ; i++) {
      ...
      //double dbeta = (beta_inf - beta_in)/(r_inf - r_n) ;
      ...
      /* Kinetic law */
      /* Modified 03/06/2017 */
      double  scrate =  InterfaceCrystalGrowthRate(s_c,s_aft,beta) ;
      double dscrate = dInterfaceCrystalGrowthRate(s_c,s_aft,beta)*dbeta ;
...
double PoreWallEquilibriumSaturationIndex(double beta_pn,double varphi_cn,double strain_n,double straind_n,double s_aft,double sc,double dt)
{
  ...
  
  {
    ...
    for(i = 0 ; i < iterations ; i++) {
      double phicrate  =  PoreCrystalGrowthRate(sc,s_aft,beta_p) ;
      double dphicrate = dPoreCrystalGrowthRate(sc,s_aft,beta_p) ;
      double pc        = CrystallizationPressure(beta_p) ;
      double dpc       = dCrystallizationPressure(beta_p) ;
      //double varphi_c  = sc*strain +  pc*sc/N_Biot +  pc*sc*sl/G_Biot ;
