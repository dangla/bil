...
#define AdsorbedChloridePerUnitMoleOfCSH(c_cl,c_oh,x) \
        (AlphaCoef(x) * (c_cl) / ((c_oh) + BetaCoef(x) * (c_cl)))
...
#define ptZ_CSH    &(HardenedCementChemistry_GetWaterSiliconRatioInCSH(hcc))
    ...
    {
      double n_cl_s = n_si_s * AdsorbedChloridePerUnitMoleOfCSH(*ptC(Cl),*ptC(OH),*ptX_CSH) ;
      Result_Store(r + i++,&n_cl_s,"n_cl_s",1) ;
    }
...
void  ComputeSecondaryVariables(Element_t* el,double t,double dt,double* x)
{
  ...
  double n_c_s      = n_cc ;
  double c_oh       = HardenedCementChemistry_GetAqueousConcentrationOf(hcc,OH) ;
  double n_cl_s     = n_si_s * AdsorbedChloridePerUnitMoleOfCSH(c_cl,c_oh,x_csh) ;
