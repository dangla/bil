  ...
  */
...

  {
    ...
    for(i = 0 ; i < np ; i++ , vim += NVI , vex += NVE) {
      ...
      for(j = 0 ; j < 3 ; j++) w_l[j] += W_Tot[j]/np ;
    ...
    for(p = 0 ; p < np ; p++) {
      ...
      /* Conservation of total mass */
      {
        ...
        /* Storage matrix */
        {
          double  dp_l2 = dui[U_p_l2] ;
          double* dxdp_l2 = ComputeVariableDerivatives(el,t,dt,x,dp_l2,I_P_L2) ;
          double  dmdp_l2 = dxdp_l2[I_M_Tot] ;
          double* c1 = c0 + 81 + 9 + 9 ;
...
//  XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
    ...
    /* Total liquid mass content and body force */
    {
      ...
      double  p_vap_M = p_vap_sat * rel_hum_M ;
      double  rho_vap = M_R_T * p_vap_M ;

      double  m_l_M  = rho_l * phi_M * sat_liq_M + rho_vap * phi_M * sat_vap_M ; // dans les macropores ;
      double  m_l_1  = rho_l * phi_m1 * solid_fraction_1 ; // dans les pellets, revient à rho_l * (initial_poro + eps1)/(1+eps1) * initial_sol_frac * (1+eps1) / (1+epsV) = rho_l * (n_0 + eps1) * phi_1_0 / (1+tr_eps)
      double  m_l_2  = rho_l * phi_m2 * solid_fraction_2 ; // dans les grains de poudre 
