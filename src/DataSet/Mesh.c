...
/* lit le maillage au format GMSH version 1.0 dans un fichier */
{
  ...
  /*
  {
    int* p0_in ;
    Node_t** p0_node ;
    p0_in = (int*) realloc(p_in,n_c*sizeof(int)) ;
    if(p0_in != p_in) arret("lit_mail_gmsh_1 (11)") ;
    p0_node = (Node_t**) realloc(p_node,n_c*sizeof(Node_t*)) ;
    if(p0_node != p_node) arret("lit_mail_gmsh_1 (12)") ;
  }
  */
  fscanf(fic_msh,"%s",mot) ;
  /* fermeture du fichier */
...
/* lit le maillage au format GMSH version 2.0 dans un fichier */
{
  ...
  /*
  p = (int*) realloc(p_in,n_c*sizeof(int)) ;
  if(p_in != p) {
    arret("impossible d\'allouer la memoire (lit_mail_gmsh_2)") ;
  }
  */
  fscanf(fic_msh,"%s",mot) ;
  /* fermeture du fichier */
...
static int**  (Mesh_CreateGraph1)(Mesh_t*) ;
extern void   (Mesh_WriteGraph1)(Mesh_t*,char*,char*) ;
static int*   (Mesh_ComputeInversePermutationOfNodes1)(Mesh_t*,char*) ;
static void   ecrit_graph(int,int,int**,char*,char*) ;
static void   sloan_graph(char*) ;

int** Mesh_CreateGraph1(Mesh_t* mesh)
/** Compute the graph matrix of node indexes.
 *  Return a pointer to an array of (n_no + 1) int* (graph).
 *  graph[i] is a pointer to int and 
 *  graph[i][...] is the list of node indexes connected to i.
 *  The nb of non zero terms of the matrix 
 ...
 **/
{
  ...
  /* Allocate memory for graph (overestimation) */
  {
    ...
    if(!graph[0]) {
      arret("Mesh_CreateGraph(2): not enough memory") ;
    }
  ...
  /* Update the graph for periodic mesh/BC */
  {
    ...
    if(periodicities) {
      Periodicities_UpdateGraph1(mesh,graph,nnz_no) ;
    }
  }
  ...
  /* We reallocate the memory for graph[0] to save space */
  {
    ...
    if(!g0) {
      arret("Mesh_CreateGraph(3): allocation impossible") ;
    }
  }
...
void   Mesh_WriteGraph1(Mesh_t* mesh,char* nom,char* format)
/** Create the graph file "nom.graph" in the format "format" */
{
  int    n_no = Mesh_GetNbOfNodes(mesh) ;
  int**  graph = Mesh_CreateGraph1(mesh) ;
  int    nnz2 = (int) (graph[n_no] - graph[0]) ;
  int    nnz  = nnz2/2 ;
  
  
  if(!strcmp(format,"metis") || !strcmp(format,"sloan")) {
    
	  ecrit_graph(n_no,nnz,graph,nom,"metis") ;
  
    if(!strcmp(format,"sloan")) sloan_graph(nom) ;
  
  } else if(!strcmp(format,"hsl") || !strcmp(format,"hsl_mc40")) {
    
	  ecrit_graph(n_no,nnz,graph,nom,"hsl") ;
  
  } else {
    
	  arret("Mesh_WriteGraph(2): unknown format") ;
  }
...
int*   Mesh_ComputeInversePermutationOfNodes1(Mesh_t* mesh,char* format)
{
  int    n_no = Mesh_GetNbOfNodes(mesh) ;
  int*   iperm = (int*) malloc(n_no*sizeof(int)) ;
  ...
  if(!strcmp(format,"hsl") || !strcmp(format,"hsl_mc40")) {
    ...
    /* Compute the row and column indexes: irn and jcn */
    {
      ...
      for(k = 0 , in = 0 ; in < n_no ; in++) {
        ...
        for(i = 0 ; i < graph[in + 1] - graph[in] ; i++) {
          int jn = graph[in][i] ;
...
void ecrit_graph(int n_no,int nnz,int** graph,char* nom,char* format)
/* ecrit le fichier graph (nom.graph) au format defini par "format"*/
{
  FILE* fic_graph ;
  ...
  /* Open the file "nom.graph" */
  {
    ...
    if(strlen(nom) + 6 > Mesh_MaxLengthOfFileName) {
      arret("ecrit_graph(1): too long name") ;
    }
    ...
    if(!fic_graph) {
      arret("ecrit_graph(2): cannot open %s\n",nom_graph) ;
    }
  }
  ...
  fprintf(fic_graph,"%d %d\n",n_no,nnz) ;
	...
	/* fichier graph au format HSL_MC40 */
  if(!strcmp(format,"hsl")) {
    int in ;
...
void sloan_graph(char* nom)
/* reecriture du fichier graph pour la methode Sloan */
{
  int    n,nnz ;
  int    *irn,*jcn ;
  char   nom_graph[Mesh_MaxLengthOfFileName] ;
  ...
  if(strlen(nom) + 6 > Mesh_MaxLengthOfFileName) {
    arret("sloan_graph(1): nom trop long") ;
  }
  
  sprintf(nom_graph,"%s.graph",nom) ;
  
  /* Read the graph */
  fic_graph = fopen(nom_graph,"r") ;
  
  if(fic_graph == NULL) {
    arret("sloan_graph(2): erreur a l'ouverture du fichier %s\n",nom_graph) ;
  }
  ...
  /* Read n and nnz */
  {
    ...
    do {
      fgets(line,sizeof(line),fic_graph);
    } while (line[0] == '%' && !feof(fic_graph));

    sscanf(line, "%d %d", &n, &nnz);
  }

  irn = (int*) malloc(nnz*sizeof(int)) ;
  jcn = (int*) malloc(nnz*sizeof(int)) ;

  /* Store indexes irn and jcn */
  {
    int    i,k ;
    ...
    for (k = 0 , i = 0 ; i < n ; i++) {
      ...
      for (;;) {
        int edge = (int) strtol(oldstr, &newstr, 10) - 1 ;
  ...
  /* Rewrite the graph in the same file */
  fic_graph = fopen(nom_graph,"w") ;
  
  if(fic_graph == NULL) {
    arret("sloan_graph(3): erreur a l'ouverture du fichier %s\n",nom_graph) ;
  }
